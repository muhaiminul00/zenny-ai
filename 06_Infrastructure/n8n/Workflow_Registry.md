...
**NOT YET VERIFIED — needs a human action, not skipped or assumed:**
- **D20 re-key/absence proof** (editing an already-synced row's key value must leave the OLD vector gone): blocked by the Viewer-only credential above (correctly so) — needs the human to change one row's `Variant SKU` in the live test sheet directly, then tell Execute to resync and check Pinecone. *(Access must be re-shared with the service account first — it's currently revoked from the 403 proof above.)*
- **Blank-key partial-failure proof (D17):** this raw export's 14 rows all had a populated `Variant SKU` — no natural blank-key row exists to exercise the partial-failure path live. Needs either a human-added blank-key test row, or an explicit decision to accept the code-level reasoning (already exercised by the blank-*whitelist*-cell case, which real rows do have) as sufficient.
