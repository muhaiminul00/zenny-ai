...
**D20 + D17 final proofs run, 2026-09-01 — STOPPED, real issues found, NOT papered over:** human re-shared access, renamed two live rows (`DPB`→`DPBP`, `TMB02`→`TMB02002`) and added one blank-key row, then a live resync ran (execution 69967). **D20 re-key proof FAILED — a genuine design gap, not a code bug:** confirmed via a disposable diagnostic workflow fetching exact vector IDs from Pinecone directly that the OLD vectors (`DPB`/`TMB02`) are still present alongside the NEW ones (`DPBP`/`TMB02002`) — a key rename leaves stale content permanently orphaned and still searchable, because the delete-then-reinsert mechanism only ever knows a row's *current* key, never a prior one. This needs a human decision (track prior keys, switch to a stable synthetic ID, or accept the limitation for now), not a unilateral Execute fix. **D17 blank-key proof structurally passed** (the row was correctly excluded, didn't corrupt the other 14 rows) **but exposed a 5th real bug:** the reported `sync_status` says `"status":"success","failed_count":0` when it should say `"partial","failed_count":1` — a stale-snapshot bug in `Aggregate & Build Status Payload` (root-caused, fix identified: derive `failed_count` as `total_rows - synced_count`), held un-applied pending the same go-ahead as the D20 decision. **PR NOT opened — Card2b's DoD is not actually complete; this is a stop-and-report per CLAUDE.md's Document Resolution Authority (D20's fix is a system-shaping decision, not a mechanical one).** Full detail: `06_Infrastructure/n8n/Workflow_Registry.md`'s Sheets Ingestion entry. Staying in Execute mode pending the human's decision on both.
