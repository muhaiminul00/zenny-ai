...
**D20 re-key proof, RUN 2026-09-01 — FAILED, real design gap found (not a code bug, a decision needed):** human renamed two live rows' `Variant SKU` (`DPB`→`DPBP`, `TMB02`→`TMB02002`), then a live resync ran (execution 69967 → sub-execution 69974). `describe-index-stats` showed the namespace grew from 28→32 records (a 4-vector *increase*, not a like-for-like replace). Built a disposable diagnostic workflow (`8D70hUnPvbM4YAzQ`, archived after) to fetch by exact vector ID from Pinecone's raw `/vectors/fetch` endpoint (the Pinecone MCP's `search-records` only works on integrated-inference indexes, not this raw one) — confirmed **both the OLD IDs (`...__DPB__0`, `...__DPB__1`, `...__TMB02__0`, `...__TMB02__1`) and the NEW IDs (`...__DPBP__0/1`, `...__TMB02002__0/1`) exist simultaneously.** The old content was never deleted; it's now permanently orphaned, stale, and still fully searchable/returnable by `Search_business_kb`.
  ...
  - **Not fixed — this needs a human decision, not a unilateral fix:** viable options include (a) track previously-synced row keys per `source_ref` (e.g., a small Data Table or a `previous_row_keys` column) and diff-delete anything missing from the current sync; (b) key vectors by a stable synthetic per-row ID instead of the mutable SKU, and store the SKU only in metadata; (c) accept this as a known limitation for now (re-keying is rare) and document it, revisiting if it causes a real incident. Each has real tradeoffs — this is exactly the kind of "shapes the system" call CLAUDE.md's Document Resolution Authority reserves for the human, not Execute's own judgment.

**Blank-key partial-failure proof (D17), RUN 2026-09-01 — structurally PASSED, but surfaced a real reporting bug (bug #5):** human added one row with a blank `Variant SKU` (whitelisted columns populated). The same resync (execution 69967 → sub-execution 69974) correctly: excluded the blank-key row from the embed/upsert pipeline (14 valid rows processed, not 15), did not write any vector for it, and did NOT abort or corrupt the other 14 rows' sync — real partial-success behavior held. **However the reported `sync_status` written to Supabase was `{"status":"success","synced_count":14,"total_rows":15,"failed_count":0,"failed_row_keys_sample":["(row 16, blank key)"]}`** — internally inconsistent: `failed_count:0` while a failed row is literally named in the same payload; `status` should read `"partial"`, not `"success"`.
  - **Root cause, found and understood, NOT yet fixed (deferred alongside the D20 decision so both land together):** `Validate Rows & Build Batch`'s code pushes `failedCount` (a primitive, copied by value at push time) onto every valid row's json as it's built via `forEach`. Because the blank-key row was the *last* row in sheet order, every valid row was already pushed (with `failedCount` still 0 at push time) before the loop ever incremented it. `Aggregate & Build Status Payload` then reads `failedCount` off `validated[0]` (the *first* valid row) — a stale snapshot from before the failure was counted. (`failedRowKeys` looks correct by comparison, but only by accident: it's a shared array reference, not a per-row copy, so it happens to reflect the final state regardless of ordering — `failedCount` has no equivalent accidental correctness.) **Correct, low-risk fix identified:** in `Aggregate & Build Status Payload`, derive `failedCount` as `totalRows - syncedCount` instead of reading it off any individual row — always exact regardless of row order, no design tradeoff. Small enough to apply immediately once the human says go; held here only because it surfaced in the same verification pass as the D20 design gap and both are being reported together.
