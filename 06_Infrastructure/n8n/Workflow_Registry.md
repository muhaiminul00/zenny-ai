...
**INPUT:** None (self-triggered). Reads `control.clients` (all rows, no status filter — `control.clients.status` is not used as a gate anywhere else in the built system, including UTIL-001 itself; every roster client is currently `onboarding` as a permanent test-fixture status, not a real in-progress state, so gating on it here would have silently excluded 100% of the real test roster and isn't consistent with any other real workflow's behavior).
