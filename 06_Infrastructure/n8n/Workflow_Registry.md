...
**UPDATED BC-076 (2026-08-31, column rename) — CORRECTION, BC-076-Card2b (2026-09-01):** the prior note above claimed this workflow's query was "updated in the same migration" to the generalized schema. **That was wrong — it was only ever updated in the DRAFT, never published.** Live-verified via `get_workflow_details` (Mandatory MCP Verification): the *active/production* version was still querying the renamed-away `notion_page_id` column — a real, undiscovered incident about to happen (last execution 2026-08-30 21:00 UTC, before the column rename; the next scheduled run at 03:00 would have 400'd, silently breaking the nightly Notion KB sync for every connected client). Root cause: this project's own recurring `update_workflow`-drafts-vs-`publish_workflow` quirk, compounded by a prior session's note asserting the fix "was updated" without actually confirming it was live. Fixed and published for real this time (`activeVersionId adab28cf-...`) before this bug could fire.

**BC-076-Card2b (2026-09-01) — generalized dispatch, not another special case:** this workflow now polls BOTH `source_type='notion'` and `source_type='google_sheets'` due rows (`?select=client_id,source_type,source_ref,source_config&source_ref=not.is.null&source_type=in.(notion,google_sheets)`) and routes each row via a new `Route by Source Type` Switch node — `notion` → `Call INT-012 Sync Notion KB` (unchanged), `google_sheets` → new `Call Sheets KB Ingestion` node calling the new Sheets ingestion sub-workflow (see entry below). **Published** (2026-09-01, `activeVersionId 90463add-...`) once the Credential Gate below cleared. **Live-verified, not assumed:** ran the full dispatcher live 3 times while debugging the Sheets leg (executions 69332/69340/69348/69356) — each run correctly fanned out to all 6 real `notion`-source clients (idempotent re-syncs, all succeeded) AND the 1 real `google_sheets`-source client in the same sweep, proving the Switch node's routing against live data, not just its wiring.
...
**FIXED (2026-08-31, BC-076-Card1 — restructured, not patched):** replaced `Search Business KB Trigger` (`executeWorkflowTrigger`, the broken `toolWorkflow`-as-`ai_tool` target) with a real `Search Business KB Webhook` (`n8n-nodes-base.webhook`, `POST /webhook/search-business-kb`, `responseMode: responseNode`) + a new `Normalize Contract` code node + a terminal `Respond to Webhook` node — exact same shape as WF-002/WF-013/WF-015's own production webhooks. All 3 Agent workflows now call it via `httpRequestTool` (matching `Check_availability`'s already-proven pattern) instead of `toolWorkflow`. **Live-verified, not assumed:** seeded 2 real facts into 2 different clients' Pinecone namespaces (Commerce-Ecom's hours, Appointment's cancellation policy), then proved via 4 real conversations: (1) Commerce-Ecom returns the exact seeded hours fact for its own client; (2) that same client asking an unrelated-namespace question does NOT leak the other client's fact (real cross-tenant isolation proof, not assumed from the code); (3) Appointment independently returns its own client's exact seeded fact, proving the `httpRequestTool` pattern works identically across archetypes; (4) Consultation's tool call succeeds and gracefully falls back when no fact is seeded for its client. All 4 tool-call traces confirmed non-null `client_id` reaching the sub-workflow. Test facts/vectors deleted, temp workflows archived, synthetic conversations purged after. **Full root cause (unchanged from the finding below, now resolved):** `toolWorkflow` nodes invoked as `ai_tool` cannot reliably resolve `$()`/`$json`/`$node[]` references to sibling main-chain nodes — a real, unresolved n8n platform limitation (confirmed via a live n8n community thread describing the identical symptom). **client_id is a static main-chain reference in every Agent (`$("<Archetype> Node Trigger").item.json.client_id`), never `$fromAI`-supplied** — a real tenant-isolation correction made before this fix shipped (an outside-voice review catch on the first draft, which would have let the LLM control tenant identity). **Flagged, intentionally not fixed here (pre-existing, inherited, not introduced by this fix):** none of `Check_availability`/`Cancel_appointment`/`Get_booking_status`, nor this new webhook, carry any auth beyond the URL itself — worth a dedicated security pass across all tool webhooks before real client traffic scales.

**Prior finding this fixes, kept for history — REAL BUG FOUND, NOT FIXED — SEVERE, blocks this tool from ever working even once ingestion is built (2026-08-31, follow-up pass):** `Search_business_kb`'s `client_id` field, mapped via `workflowInputs` (`defineBelow`) as a standard cross-node n8n expression (`$('NodeName').item.json.client_id`), was live-verified to resolve to **`null` on every single call** — confirmed via the sub-workflow's own trigger data across a real Commerce-Ecom conversation. This means the tool always queries Pinecone under an empty/wrong namespace and can never return real client-scoped content, regardless of how much KB content gets ingested later. **6 independent fix attempts, all reproduced the same null, ruling out a typo or a one-off:** `$('Node').item.json.field`, `$('Node').first().json.field`, bare `$json.field`, legacy `$node["Node"].json["field"]`, `$fromAI(...)` with the value stamped into the system prompt for the LLM to echo back, and the same `$fromAI` approach with the tool's own `description` field made explicit about the requirement — none reached the sub-workflow with a non-null `client_id`. Root cause appears to be a genuine n8n platform limitation: a `toolWorkflow` node invoked as an AI Agent tool (`ai_tool` connection) cannot reliably resolve any of `$()`/`$json`/`$node[]`/`.first()` references to sibling main-chain nodes — confirmed the SAME static-expression pattern resolves correctly for ordinary main-chain nodes in the identical workflow (e.g. `Get Business Name`), so the failure is specific to the ai_tool evaluation boundary, not a broken node name or a pairedItem/branching artifact. `$fromAI(...)` for the `query` field is the only channel confirmed to actually deliver a value across that boundary. **Not fixed this pass** — the likely real fix needs deeper investigation than a "small scoped pass" allows: either `toolWorkflow`'s `source: 'parameter'` mode with a single raw-JSON expression body (untested — may use a different evaluation path), or restructuring `Search_business_kb` as an inline `httpRequestTool` (matching `Check_availability`'s proven-working architecture) instead of a shared `toolWorkflow` sub-execution — the latter would mean duplicating the Pinecone-query logic into all 3 Agent workflows instead of one shared tool, a real design tradeoff for whoever picks this up. Commerce-Ecom's `Search_business_kb` node was left in the `$fromAI` + prompt-injection + explicit-description state (harmless, not worse than the original, documents the attempted mechanism); Appointment/Consultation's copies were left untouched in their original (also-broken) state. **Flag for D9/launch gate: this must be fixed and re-verified with a real grounded answer before BC-076 clears any archetype's launch gate — a wired-but-inert tool is not "verified," per D6's explicit standing.**

---

### Zenny Runtime - Sheets KB Ingestion (BC-076-Card2b, D20 fix = BC-076-Card2c)
**n8n ID:** `KR0kHvk3kJRThrX5` · **published**, active (2026-09-01, `activeVersionId 50aa506d-eb55-43e9-9eef-1ae5f44d1726`) — Credential Gate cleared, D20 rename-safety fix live-verified end to end

**PURPOSE:** BC-076-Card2b's Google Sheets ingestion leg, service account per D11 — lets a client's product/FAQ list living in a real Google Sheet feed `Search_business_kb` the same way the Notion leg already does. Called by `SCH-004`'s new dispatcher (see its updated entry above) for every `source_type='google_sheets'` row in `control.client_kb_source`.

**TRIGGER:** Execute Workflow Trigger (Define Below): `client_id`, `source_ref` (`{spreadsheetId}:{sheetName}`, D15), `source_config` (`{key_column, whitelist_columns[]}`, D18).

**FLOW:** `Parse Source Ref` (splits `source_ref`, validates `source_config` is present) → `Read Sheet Rows` (native `n8n-nodes-base.googleSheets`, `sheet.read`, `authentication: serviceAccount` — **credential intentionally unattached, see Credential Gate below**) → `Validate Rows & Build Batch` (header-column check against `key_column`+whitelist per D21; per-row: blank key column = failure per D17, blank whitelist column = just less content, not a failure — a deliberate tightening of the original spec wording, see note below) → `Has Valid Rows?` (IF, guards the real "0 valid rows still writes a real status" gap rather than silently skipping — the exact class of bug D12 exists to catch) → true: `Loop Rows` (`splitInBatches`, batch 1) per row: `Compute Delete IDs` (20-ID cap) → `Delete Existing Row Vectors` → `Chunk Content` (~700 char, paragraph-packed, matches the Notion leg) → `Embed Chunk (OpenRouter)` → `Upsert Chunk Vector` (composite ID `google_sheets__{source_ref_safe}__{row_key_safe}__{chunk_index}` + full metadata per D19) → loop; both branches converge on `Aggregate & Build Status Payload` → `Write Sync Status (RPC)` (`upsert_client_kb_source`).

**Design corrections made during the build, not just the plan (real, not cosmetic):**
- **Delete-by-ID instead of delete-by-metadata-filter (D20's actual implementation):** Pinecone's own docs didn't clearly confirm metadata-filtered delete is supported on serverless indexes (the tier `zenny-business-kb` uses) — rather than build on an unverified claim, deletion instead targets a capped set of deterministic candidate IDs (`chunk_index` 0-19) by ID, which is universally supported and needs no filter capability at all. Same D20 outcome (stale chunks from a shrinking/edited row are cleared before reinsert), safer mechanism.
- **`source_ref_hash` → plain sanitized `source_ref` string:** the locked spec's composite-ID wording used a hash; implemented as a direct sanitized string instead (no crypto-module dependency to verify in n8n's Code node sandbox — an unnecessary unverified assumption avoided, same uniqueness guarantee).
- **`content_hash` metadata field dropped**, `synced_at` timestamp kept instead — D19's stated purpose (debugging/cleanup) is fully served by the delete-then-reinsert mechanism (D20) and the timestamp; a hash added a second unverified crypto dependency for no operational gain in this design.
- **D17's "blank cell" trigger narrowed to the key column only:** a blank *whitelisted content* column (e.g. an empty "notes" field) is legitimate real-world spreadsheet data, not a failure — only a blank *key* column is unrecoverable (can't identify/dedupe/delete against). The spec's own verification-plan wording ("leave a whitelisted-column cell blank") is corrected here to mean the key column specifically.

**Credential Gate (Standing Rule) — resolved 2026-09-01.** Human created a real GCP service account (`zenny-sheets-reader@zeromanual-production.iam.gserviceaccount.com`), built the n8n credential (`list_credentials` confirms: id `ERpU1vbQ14oSyPFD`, name "Google Service Account API", type `googleApi` — exactly the type this node needs, confirming the fifth-pass auth-mode verification held), and shared a real raw Shopify product-catalogue export (`products_export_1` tab, spreadsheet `1N63aUksP8ihKEZuQdvAsSFkHuEX4eXKTY5NQIL8KYec`) Viewer-only with that service account. Attached via `setNodeCredential`, then live-verified the credential actually authenticates by pulling the sheet's real header row through the node's own `getSheetHeaderRowWithGeneratedColumnNames` load-options method (a live probe, not an assumption) before building anything on it.

**Key column / whitelist resolution (D14/D21) — a real trap caught before it shipped:** the header row is a standard raw multi-variant Shopify export, where `Handle` repeats across every variant row of the same product — picking it as the key column (the natural-looking choice) would have made D20's delete-then-reinsert silently clobber every variant but the last-synced one under one shared row key. Flagged to the human rather than guessed; confirmed `Variant SKU` (genuinely row-unique) as the key column, and a "Core + Price + all Shopify metafield columns + Image Src + Image Alt Text" whitelist (28 columns) — the image columns are ingested as plain text (URL/alt-text string), not visual search; the human separately asked for real image-based product search + a recommendation carousel, explicitly deferred as a new, not-yet-scoped Build Card (needs its own vision-embedding/index-strategy design, out of Card2b's approved scope).

**REAL DEPENDENCIES:** `zenny-pinecone-api`, `openrouter-zm`, `zenny-vault-suparbase` (all pre-existing), `googleApi` credential "Google Service Account API" (`ERpU1vbQ14oSyPFD`, new, Credential Gate above).

**REAL SCHEMA CHANGE (BC-076-Card2b):** `control.client_kb_source` gained a `source_config jsonb` column (D18) — holds `key_column`/`whitelist_columns` per source row, generalizing to whatever future legs (Card 3) need without further overloading `source_ref`. `upsert_client_kb_source` RPC extended with a 5th `p_source_config` parameter — **applied the project's own logged pitfall correctly this time:** the old 4-arg signature was explicitly `DROP`ped before `CREATE OR REPLACE`, and the resulting ACL was checked and corrected (the schema-level default grant had re-added `anon` EXECUTE on the new signature — revoked, leaving only `authenticated`/`service_role`, matching the original's intent). Provisioned live for the real demo client "Carmelli Bakery" (`eb27a21f-209d-4b6d-8f6e-cb216411f6c4`, `commerce_ecom`) via a direct `select public.upsert_client_kb_source(...)` call — same demo store already used for BC-076-Card1's Notion leg, now carrying both KB sources simultaneously (proves D-series' multi-source-per-client design actually holds, not just on paper).

**4 REAL BUGS FOUND AND FIXED, live execution (not caught by `validate_workflow` — none of these are structural/type errors, only surface at runtime against real data):**
1. **`Has Valid Rows?` branch logic inverted:** the IF condition tested `__no_valid_rows === true` but the TRUE branch was wired to `Loop Rows` and FALSE to `Aggregate & Build Status Payload` — backwards from the node's own name. As originally built, every real valid row would have skipped the entire embed/upsert pipeline and gone straight to a fake "success" status write with zero vectors ever created. Fixed by flipping the condition to `!== true` (matches "has valid rows" literally) rather than rewiring connections.
2. **Same node, a second real crash:** `typeValidation: 'strict'` + `looseTypeValidation: false` threw `Wrong type: '' is a string but was expecting a boolean` on this exact condition shape — root cause was the condition's leftover placeholder `rightValue: ""` (a string) never used by the unary boolean operator, but still type-checked under strict mode. Fixed per n8n's own suggested remedy: `typeValidation: 'loose'` + `rightValue: true` (a real boolean, not a placeholder).
3. **`Chunk Content` per-item/all-items mode mismatch:** mode was `runOnceForEachItem` but the code returned an *array* of chunk objects — that mode requires returning exactly one `{json:...}` object per input item; returning an array produced a malformed `json` field (`"A 'json' property isn't an object"`). Fixed by switching to `runOnceForAllItems` and iterating `$input.all()`.
4. **The one that actually mattered most — a false "success" status with zero real vectors written:** after fixing 1-3, a full live run reported `{"status":"success","synced_count":14,"failed_count":0}` in Supabase, but every single `Upsert Chunk Vector` call had actually failed (`"Vector dimension 0 does not match the dimension of the index 1536"`), silently swallowed by `onError: continueRegularOutput`. Root cause: `Delete Existing Row Vectors` 404'd (expected and correct on a first sync — nothing to delete yet), but when an HTTP node's request fails and `continueRegularOutput` catches it, n8n replaces that item's entire `json` with `{error:{...}}`, wiping the row's `content`/`row_key_safe` fields that `Chunk Content` needed. Empty content → empty `chunk_text` → OpenRouter rejected it → empty embedding → Pinecone rejected the 0-dim vector → also swallowed → loop kept going, reporting clean success. **Fixed by sourcing row data from `$('Compute Delete IDs').all()`** (a Code node, always intact) instead of the immediate input, which can be wiped by the preceding fallible HTTP call. Re-verified: 28 real vectors landed in Pinecone (`describe-index-stats`, namespace `eb27a21f-...` → `recordCount: 28`), matching the real `upsertedCount:1` response on every one of the 28 upsert calls — this time the "success" status is actually true, confirmed independently, not just trusted from the workflow's own report.

**LAST VERIFIED, 2026-09-01 — live, not `test_workflow`-pinned (would have auto-pinned every credentialed/HTTP node, defeating the point):**
- Full pipeline, real data: 14/14 real Shopify variant rows synced, 28 real chunks embedded (`text-embedding-3-small`) and upserted to Pinecone, confirmed independently via `describe-index-stats` (not just trusted from the workflow's own status write).
- Full dispatcher proof: `SCH-004`'s generalized Switch correctly routed both the `notion` leg (6 real clients) and the `google_sheets` leg (this client) in the same live sweep — 3 full sweeps run while debugging (69332→69340→69348→69356), each one a genuine production-shaped execution, not a simulation.
- End-to-end retrieval proof: called `Search_business_kb`'s real webhook directly with `{client_id: "eb27a21f-...", query: "Do you have a Curren watch for men?"}` — got back the exact seeded product content (top match score 0.76, correct `row_key`/`source_ref` metadata), proving the full embed→Pinecone→format chain actually surfaces sheets-ingested content, not just that vectors exist.
- Service-account write-permission boundary confirmed correctly minimal: attempted a live D20 re-key test via a temporary throwaway workflow (`UpOjx2Vb3r2N8Kta`, archived after) using the same credential in `update` mode — got a clean `403 PERMISSION_DENIED` ("The caller does not have permission"), because the credential is (correctly, per the setup instructions given) Viewer-only. This is the right security posture, not a bug — Card2b's design never needs write access to a client's sheet.
- **403-vs-404 revoked-access error-status proof, CLOSED 2026-09-01:** human temporarily unshared `products_export_1` from `zenny-sheets-reader@...`, then a real SCH-004 sweep was run live (execution 69959 → sub-execution 69966). `Read Sheet Rows` failed with a clean, distinguishable error: `403 - {"error":{"code":403,"message":"The caller does not have permission","status":"PERMISSION_DENIED"}}`, surfaced by n8n as `NodeApiError` / "Forbidden - perhaps check your credentials?". No crash, no silent no-op, no false-success write — the workflow failed before reaching `Aggregate & Build Status Payload`, so `client_kb_source.sync_status` in Supabase was correctly left untouched (stale, not falsely overwritten) rather than written with a bogus success/failure payload.
  - **New finding, not a Card2b defect but worth flagging:** `Call Sheets KB Ingestion` on SCH-004 runs `mode: 'each'` with `waitForSubWorkflow: true` and no `continueOnFail`/error-output wiring, so this one client's 403 propagated up and marked the *entire* SCH-004 parent execution `status: error` — even though the parallel Notion branch (6 clients) completed successfully in the same run. Today this only affects the run's own status flag (data-wise nothing was corrupted, and a second `google_sheets` client, if one existed, would also have been skipped by the same failure since `each` mode stops iterating on first error). Not fixed here — out of this card's scope — but real, and worth a small follow-up (per-item `continueOnFail` on that node) before more clients are on the Sheets leg, since one client's revoked access would currently mask whether every other Sheets client that night synced cleanly.

**D20 re-key proof, RUN 2026-09-01 — FAILED, real design gap found (not a code bug, a decision needed):** human renamed two live rows' `Variant SKU` (`DPB`→`DPBP`, `TMB02`→`TMB02002`), then a live resync ran (execution 69967 → sub-execution 69974). `describe-index-stats` showed the namespace grew from 28→32 records (a 4-vector *increase*, not a like-for-like replace). Built a disposable diagnostic workflow (`8D70hUnPvbM4YAzQ`, archived after) to fetch by exact vector ID from Pinecone's raw `/vectors/fetch` endpoint (the Pinecone MCP's `search-records` only works on integrated-inference indexes, not this raw one) — confirmed **both the OLD IDs (`...__DPB__0`, `...__DPB__1`, `...__TMB02__0`, `...__TMB02__1`) and the NEW IDs (`...__DPBP__0/1`, `...__TMB02002__0/1`) exist simultaneously.** The old content was never deleted; it's now permanently orphaned, stale, and still fully searchable/returnable by `Search_business_kb`.
  - **Root cause, structural, not a bug to patch:** `Compute Delete IDs` derives the candidate delete-IDs to purge from the row's *current* key on every sync (`google_sheets__{source_ref_safe}__{row_key_safe}__0..19`). When a key changes, the sync has no way to know the row *used to be* `DPB` — nothing in any future run ever again computes an ID containing `DPB`, so those vectors are never targeted for deletion by design, not by accident. D20's "delete-then-reinsert per row key every sync" only ever cleans up a row's own vectors *under its current key* — it silently assumed the key never changes, which the human's own D14 trap (Handle vs Variant SKU) already flagged as a real-world risk category.
  - **Decision made, 2026-09-01 — human chose: track previously-synced row keys.** Add a mechanism (Data Table or a persisted `previous_row_keys` field per `source_ref`) that remembers the last sync's full key set, diffs it against the current sync's keys, and explicitly deletes any key present last time but missing now (covers rename, delete, and reorder cases — not just rename). **NOT YET BUILT.** Per CLAUDE.md's Commander→gstack→Execute planning bridge, this is real new architecture (a new persistence layer + diff logic + edge cases: multiple renames in one sync, backfill for the 14 rows already synced before this existed, what happens to orphans already in Pinecone from the DPB/TMB02 test above) — it needs its own `/plan-eng-review` pass before a Build Card, not a freehand Execute build. Tracked as a follow-up card, not yet scheduled.
  - **Known residual data:** the `DPB`/`TMB02` orphan vectors created during this proof run are still live in Pinecone (`...__DPB__0/1`, `...__TMB02__0/1`, namespace `eb27a21f-...`) — real test data, not synthetic, left in place deliberately pending the fix above (a manual one-off delete would just mask the gap this proof was meant to surface). Clean up as part of the follow-up card, not before.

**Blank-key partial-failure proof (D17), RUN 2026-09-01 — CLOSED, structurally passed AND the reporting bug is fixed + re-verified:** human added one row with a blank `Variant SKU` (whitelisted columns populated). The first resync (execution 69967 → sub-execution 69974) correctly excluded the blank-key row from the embed/upsert pipeline (14 valid rows processed, not 15), wrote no vector for it, and did NOT corrupt the other 14 rows' sync — real partial-success behavior held. **But surfaced bug #5:** the reported `sync_status` was `{"status":"success","synced_count":14,"total_rows":15,"failed_count":0,"failed_row_keys_sample":["(row 16, blank key)"]}` — internally inconsistent (a failed row named, yet `failed_count:0`).
  - **Root cause:** `Validate Rows & Build Batch`'s code pushes `failedCount` (a primitive, copied by value at push time) onto every valid row's json as it's built via `forEach`. Because the blank-key row was the *last* row in sheet order, every valid row was already pushed — with `failedCount` still 0 at push time — before the loop ever incremented it. `Aggregate & Build Status Payload` then read `failedCount` off `validated[0]` (the *first* valid row), a stale pre-failure snapshot. (`failedRowKeys` happened to look correct only because it's a shared array reference, not a per-row copy — `failedCount`, a primitive, has no equivalent accidental correctness.)
  - **Fixed and re-verified live, 2026-09-01:** `Aggregate & Build Status Payload` now derives `failed_count` as `total_rows - synced_count` (exact regardless of row order) instead of reading any individual row's own field. Published (`activeVersionId: 2b5397d6-d37d-40c0-95f4-454aa1605da9`). Re-ran the full SCH-004 sweep (execution 69976) and independently queried `control.client_kb_source` directly (not trusting the workflow's own report): `sync_status = {"status":"partial","synced_count":14,"total_rows":15,"failed_count":1,"failed_row_keys_sample":["(row 16, blank key)"]}` — now internally consistent and correct. Pinecone `recordCount` unchanged at 32 (idempotent re-run of the same 14 rows), confirming the fix didn't disturb anything else.

**D20 rename-safety fix — BC-076-Card2c, D24-D32, CLOSED 2026-09-01 (same-day follow-up).** Full pass: `gstack /plan-eng-review` (locked the architecture, including 2 cross-model findings from the outside-voice pass — see decisions below) → Execute built it same session, live-verified end to end, published (`activeVersionId 50aa506d-eb55-43e9-9eef-1ae5f44d1726`). Full plan: `C:\Users\muhai\.claude\plans\starry-snuggling-patterson.md` (local, not repo-committed — decisions captured here and in the Wiki instead).

- **D24 — orphan-key source of truth is Pinecone's own `GET /vectors/list`, not a new Supabase column.** At the start of each sync, lists all vector IDs under `prefix=id_prefix(source_ref)` (paginated via n8n's built-in HTTP pagination, live-verified against the real index: 7 real pages fetched, 32 IDs aggregated correctly), parses each into `row_key_safe` by stripping the shared `id_prefix` (D28) and taking everything before the last `__` as the key (chunk index after it is always a plain integer, so this split is unambiguous even if a key itself contains `__`). Diffs against `sheetRowKeys` (every key physically present in the sheet this run, valid-or-not — D31) to compute `orphanKeys`. Chosen over a self-maintained tracking column specifically because Pinecone's own vectors can't drift from themselves the way a separately-written column could after a partial-failure sync, and it auto-discovered the two known orphans with zero backfill step.
- **D25 — formally supersedes D16** (cross-leg row deletion, previously deferred to a future Card 3 mechanism). A key missing from `sheetRowKeys` is indistinguishable between "renamed" and "deleted entirely" given D14's key-only row identity — no second signal exists to tell them apart without inventing new state purely to preserve a distinction with no user-facing value. One mechanism now closes both; D16 is retired, not deferred.
- **D26 — malformed/unparseable listed IDs are skipped and counted, never auto-deleted** (`orphans_skipped_malformed`, with a capped sample of the actual IDs, not just a count).
- **D27 — list/delete-call failure degrades gracefully, matching D17's existing posture:** a failed list skips orphan cleanup for that cycle only (`orphan_step_skipped: true`) and the normal per-row ingest still runs; a failed delete batch is logged per-key (`orphan_delete_failed_keys`) and retried automatically next cycle, since the source of truth (Pinecone itself) never goes stale. **Scope trim, disclosed:** the plan's `consecutive_orphan_step_failures` cross-run counter was NOT built this pass — it would need a new Supabase read of prior `sync_status` for marginal value over the per-run signals already built; folded into the "wire alerting" TODO below instead of adding that read now.
- **D28 — one shared `id_prefix` (computed once in `Parse Source Ref`, threaded through `Validate Rows & Build Batch` per row) replaces the previously-duplicated `'google_sheets__' + source_ref_safe + '__'` literal** in `Compute Delete IDs`, `Chunk Content`, the new list/parse logic — a real DRY fix, not just new-code hygiene.
- **D29 — orphan-deletion is gated on a non-empty real read** (new `Verify Read Completeness` node, checking real row count after `Read Sheet Rows` was set `alwaysOutputData: true`). **A pre-existing latent gap this also closed, found live during the build, not just theorized:** without `alwaysOutputData`, n8n's zero-item-safety default meant a Sheets read that succeeded with 200 OK but returned 0 rows would have silently skipped `Validate Rows & Build Batch` and every node after it — no error, no status write, nothing. A genuine `NodeApiError` (403/404/503) was already proven fail-loud earlier this session; this closes the *other* half (a "successful" empty read) that was never actually tested. Simpler than the plan's original phrasing (which conditioned the gate on prior-sync history); gating on real-row-count-greater-than-zero gives the identical protection with no extra state, since a brand-new empty source has an empty `priorKeys` anyway.
- **D30 — pagination capped** (`maxRequests: 50` for production headroom; the pagination *mechanism* itself — parameter update + completion detection — was live-verified against the real index across 7 real pages, but that 50-page ceiling was never itself approached, let alone stress-tested) **and deletes are batched** to Pinecone's documented 1000-ID-per-request limit (not live-tested at that scale — current volume is 4 IDs — but the batching code is in place and correct).
- **D31 — a row present in the sheet but failing this cycle's validation keeps its last-known-good vectors untouched, explicitly** (not an accidental side effect of D24's diff) — matches D17's existing "continue past a bad row, don't corrupt" philosophy.
- **D32 — a duplicate `row_key` across two sheet rows excludes BOTH from the valid-upsert batch**, reported via the existing `failed_row_keys` mechanism (not a new channel) — closes a real clobber/mis-diff risk an outside-voice pass caught in the first draft ("reported" alone wasn't "excluded").

**Outside-voice (Codex) cross-model findings, both resolved with the human before build, not silently adopted or dismissed:** (1) the first draft rebuilt candidate delete-IDs (`0..19`) for orphaned keys instead of deleting the *exact* IDs `/vectors/list` already returned — fixed, D24 now deletes exact listed IDs, closing a real "row produces 20+ chunks" edge case the rebuild approach couldn't have caught. (2) **The most important one:** the first draft had no guard against an incomplete Sheets read — a truncated/permission-limited/API-hiccup read that doesn't hard-error would have made every unread row look orphaned and deleted real, current KB content. This became D29's read-completeness gate, and finding the *pre-existing* zero-item-safety gap while implementing it (see D29 above) is a direct result of taking that finding seriously rather than building the minimum literal fix.

**LIVE VERIFICATION, 2026-09-01 — the actual D20 proof this whole fix exists for, run for real, not simulated:**
1. Live-verified `GET /vectors/list` (prefix+namespace filtering, pagination round-trip across 2 real pages with zero overlap) and n8n's built-in HTTP pagination feature (7 pages, 32 IDs aggregated correctly) against the real `zenny-business-kb` index before building anything on them.
2. **Full real SCH-004 sweep (execution 70124 → sub-execution 70131) after publishing the fix:** `Verify Read Completeness` → `real_row_count:15, read_gate_passed:true`. `List Previously-Synced Vector IDs` → 32 vectors. `Compute Orphaned Row Keys` → correctly identified **exactly** `DPB__0/1` and `TMB02__0/1` as orphaned (4 IDs, 0 malformed) — zero false positives against the other 14 valid current keys (including `DPBP` and `TMB02002`, the renamed replacements, both correctly left untouched). `Delete Orphaned Vectors` → succeeded. `sync_status` written: `{"status":"partial","synced_count":14,"total_rows":15,"failed_count":1,"orphans_found":4,"orphans_deleted":4,"orphan_delete_failed_keys":[],"orphans_skipped_malformed":0,"orphan_step_skipped":false}`.
3. **Independently re-verified, not trusted from the workflow's own report:** a fresh, separate `GET /vectors/list` call against the real index afterward returned exactly 28 vectors (down from 32) — `DPB`/`TMB02` genuinely gone, all 14 valid keys' 28 vectors intact. This is the exact evidence deliberately left in Pinecone since the original D20 proof, now closed.
4. **Not individually live-fired this pass, by design choice, not oversight:** the D32 duplicate-key branch, the D26 malformed-ID branch, the D27 list/delete-failure branches, **and — the one that matters most — D29's actual BLOCK path** (what happens when the read-completeness gate fails and refuses to run orphan-deletion) are all implemented and code-reviewed but weren't each deliberately triggered against the live production sheet/index. The one production run this session exercised D29's PASS path only (`real_row_count:15, read_gate_passed:true`, a normal successful read) — the gate blocking on a genuinely incomplete read, the exact behavior D29 exists to guarantee, has not itself been live-fired. Flagged here plainly, not glossed over, precisely because it's the highest-stakes untested path, not a minor one. (Review math, for anyone cross-checking: gstack `/plan-eng-review`'s own pass raised and resolved 6 decisions interactively; its outside-voice (Codex) pass then surfaced 16 additional raw findings, of which 2 were escalated to the human as genuine cross-model tensions and the other 14 were folded in directly as unambiguous corrections — 6 + 2 = 8 explicit human-facing decisions across the whole review, all resolved; the 14 auto-folded findings are captured in the D24-D32 decisions above, not separately re-counted.)

**Follow-ups tracked, not built this pass (see `TODOS.md`):** a periodic cross-source Pinecone orphan audit (closes the "dormant source never re-syncs" gap D24's ordinary-sync-path design can't reach on its own), and wiring `sync_status` failures (including the deferred consecutive-failure counter) to real alerting once this project has any.

**BC-076-Card3 note (2026-09-02): this workflow was NOT migrated to call the new Generic KB Ingestion Core (D25), even though D25's shared-core extraction was ported FROM this workflow's own logic.** A deliberate scope trim, disclosed not silently dropped: the new Shopify/WooCommerce legs (below) already avoid duplicating the hardening by calling the shared core directly, which is D25's real goal (no new drift). Retrofitting this already-shipped, working production workflow to also delegate to the shared core is real refactor risk (a regression on live Sheets ingestion) that doesn't block Card 3's own Definition of Done — tracked in `TODOS.md` as its own fast-follow, with Codex's own recommended safety steps (freeze/export first, dual-run comparison against the same real sheet pre/post-refactor) attached to that item.

---

### Zenny Runtime - Generic KB Ingestion Core (BC-076-Card3, D25)
**n8n ID:** `XxkqBACpoJiifl0T` · **published**, active (sub-workflow only, no production trigger — called by Sheets/Shopify/WooCommerce fetch workflows... currently just Shopify/WooCommerce, see note above)

**PURPOSE:** D25's shared generic ingestion core — the chunk/embed/upsert/orphan-cleanup logic extracted from `KR0kHvk3kJRThrX5` (Sheets KB Ingestion) and generalized so new ingestion legs (Shopify, WooCommerce, and future ones) reuse the exact same hardening (D19/D20/D24-D32) instead of each re-implementing it — the drift that broke the Notion leg (see Card3b, tracked separately) is the exact failure mode this extraction exists to prevent from happening again.

**TRIGGER:** Execute Workflow Trigger (Define Below): `client_id`, `source_type`, `source_ref`, `read_complete` (boolean, caller-asserted), `records` (array of `{row_key, content}` — already fetched/normalized by the caller).

**FLOW:** `Parse Contract` (computes `id_prefix = source_type + '__' + sanitize(source_ref) + '__'` — carries BOTH source_type and source_ref, not source_type alone, per Codex's source-instance-collision catch during Card3's `/plan-eng-review`) → `Read Gate Passed?` (IF on `read_complete`): **true** → `List Previously-Synced Vector IDs` (Pinecone `/vectors/list`, prefixed+paginated) → `Compute Orphaned Row Keys` (diffs against `records`, D24/D26 logic unchanged) → fans out to `Batch Orphan Deletes`→`Delete Orphaned Vectors` AND directly to `Validate Rows & Build Batch`; **false** → `Validate Rows & Build Batch` directly (orphan cleanup skipped this cycle, per D29/D30 — a partial/failed read never triggers a deletion). `Validate Rows & Build Batch` → `Has Valid Rows?` → true: `Loop Rows` (per row: `Compute Delete IDs`→`Delete Existing Row Vectors`→`Chunk Content`→`Embed Chunk`→`Upsert Chunk Vector`, D19/D20 unchanged) → `Aggregate & Build Status Payload` → `Write Sync Status (RPC)`.

**Real fan-in/fan-out topology, not expressible in one SDK builder chain:** `Compute Orphaned Row Keys` feeds BOTH `Batch Orphan Deletes` and `Validate Rows & Build Batch` (one node, two downstream targets) — built via `create_workflow_from_code` for the primary chain, then patched with one explicit `addConnection` operation for the second wire, mirroring exactly the "explicit addConnection, not `updateNodeParameters` alone" gotcha already documented for switch nodes elsewhere in this project (same underlying lesson: connections need explicit operations, not implied ones).

**One real correctness gap found and fixed during Card3's first live end-to-end test, not caught by any static check:** a genuinely empty source (0 rows, read succeeded) and a source whose read failed entirely (0 rows because the fetch itself broke) both produced `total_rows:0, failed_count:0, status:'success'` — indistinguishable. Live-reproduced for real against WooCommerce's known-flaky test store (see its own entry below): a real non-JSON response correctly produced `read_complete:false, records:[]`, but the written `sync_status` still read `{"status":"success","total_rows":0}` — misleading for an actual failure. **Fixed:** `Aggregate & Build Status Payload` now reports `status:'failed'` when `read_complete:false` and zero rows were processed, and includes `read_complete` itself in the written payload for downstream visibility. **Re-verified with a fresh run during `/qa`, not just trusted from the publish action:** the first re-check after publishing still showed the OLD pre-fix payload (`client_kb_source` hadn't been re-synced since the fix landed — a real "claimed but not yet re-proven" gap caught by re-checking rather than assuming). A fresh SCH-004 sweep (execution 70582 → sub-execution 70595) genuinely re-hit the same live WooCommerce failure and this time wrote `{"status":"failed","read_complete":false,...}` — the fix is now actually proven against real data, not just published. Same sweep also re-synced Shopify (execution 70592, same 14/14 products) with zero duplication (`describe-index-stats` held steady at 42 records both before and after), confirming D19/D20's delete-then-reinsert idempotency holds under a repeat real sync, not just a first one.

**REAL DEPENDENCIES:** `zenny-pinecone-api`, `openrouter-zm`, `zenny-vault-suparbase` (all pre-existing, same as Sheets KB Ingestion — no new credentials for this extraction).

**LAST VERIFIED, 2026-09-02 — live, real catalog data, not synthetic:** called by the Shopify leg (14 real products, 14 chunks, 14 real upserts — see its own entry) and the WooCommerce leg (0 records, correctly a no-op with the corrected `status:'failed'` reporting). Independently confirmed via `describe-index-stats`: `zenny-business-kb` namespace `eb27a21f-...` grew from 32→42 records (the pre-existing 28 Sheets vectors — 32 minus D20's 4 cleaned-up orphans, per that workflow's own history — plus 14 new Shopify vectors, zero collisions, zero data loss).

---

### Zenny Runtime - Shopify KB Ingestion (BC-076-Card3)
**n8n ID:** `a7VNICxO5vPAp034` · **published**, active (sub-workflow, called by `SCH-004`'s `shopify` branch)

**PURPOSE:** Card3's Shopify leg — fetches a client's published, in-stock Shopify products and feeds them to the Generic KB Ingestion Core (above), so a client's real Shopify catalog becomes searchable via `Search_business_kb`. The first real exercise of Shopify's product API anywhere in this project (`Wiki/credentials/shopify.md` had disclosed this as never-yet-exercised beyond the connection layer).

**TRIGGER:** Execute Workflow Trigger (Define Below): `client_id`.

**FLOW:** `Resolve Shopify Credential (UTIL-006)` (category `ecommerce` — see the live bug found below) → `Credential Available?` → false: throws a clear error; true: `Fetch Shopify Products (GraphQL)` (Admin API `2026-07`, D28 — live-verified as the current stable version via WebSearch before hardcoding it, not guessed; `X-Shopify-Access-Token` header with UTIL-006's resolved token, not `Authorization: Bearer`; `query: "status:active"` server-side filter, `first: 250` single-page fetch) → `Normalize Products` (strips HTML from `descriptionHtml`, builds a normalized-product content string per D26 — title/description/brand/category/tags/handle/variant SKU+price — filters to any-variant-`availableForSale` or `totalInventory>0` per D29; sets `read_complete = !hasNextPage`, and treats a GraphQL `errors` array or missing `data.products` as a failed read, not a crash) → `Call Generic Ingestion Core`.

**D28 (GraphQL over REST) — a real, disclosed shortcut, not full cursor pagination:** fetches `first: 250` in one page. If a real catalog exceeds 250 products, `hasNextPage:true` sets `read_complete:false`, which makes the shared core skip orphan cleanup that cycle (never risks deleting vectors for unseen products) while still ingesting the 250 it did fetch. Real cursor-based multi-page traversal is a disclosed fast-follow, not built this pass — test-client catalogs are well under this ceiling (Carmelli's real catalog: 14 products, confirmed live).

**One real bug found and fixed via live testing, not caught by `validate_workflow` (shared with the WooCommerce leg below, same root cause):** `Resolve Shopify Credential (UTIL-006)` initially passed `category: 'shopify'` — wrong. `get_client_connection`'s `p_category` filters `control.client_connections.category`, a functional grouping (`'ecommerce'`), not the provider name. UTIL-006 correctly returned `available:false` for Carmelli's real, connected, working Shopify account on the first live test — fixed to `category: 'ecommerce'`, re-verified live.

**LAST VERIFIED, 2026-09-02 — live, real catalog, not synthetic (execution 70527 → sub-execution 70535):**
- 14 real products fetched from `ember-and-co-ozearycd.myshopify.com` via a real GraphQL call (`requestedQueryCost:112, actualQueryCost:5`, well under Shopify's throttle budget), all published + in-stock, correctly normalized (HTML stripped, brand/handle/variant/price all present in the resulting content string).
- All 14 passed to the Generic Ingestion Core; `sync_status` written: `{"status":"success","synced_count":14,"total_rows":14,"failed_count":0}`.
- **Independently re-verified, not trusted from the workflow's own report:** `describe-index-stats` on `zenny-business-kb` showed namespace `eb27a21f-...` at 42 records (28 pre-existing Sheets vectors + 14 new Shopify vectors, exact expected delta).
- **Real end-to-end retrieval proof:** called `Search_business_kb`'s live webhook directly (`curl`, not simulated) with `{client_id: "eb27a21f-...", query: "Do you have a Curren watch for men?"}` — got back the exact seeded Shopify product content (title, description, brand, SKU, price) verbatim among the top matches, alongside the same product's pre-existing Notion/Sheets-sourced content (Carmelli Bakery is a multi-source demo client by design, not a data-integrity problem).
- **Cross-tenant isolation confirmed:** the identical query against Client A (`baa673b5-...`, zero Shopify data) correctly returned the honest "no client-specific knowledge base content is available yet" fallback, zero leakage from Carmelli's namespace.

---

### Zenny Runtime - WooCommerce KB Ingestion (BC-076-Card3)
**n8n ID:** `omhitVMzXB5jXE8A` · **published**, active (sub-workflow, called by `SCH-004`'s `woocommerce` branch)

**PURPOSE:** Card3's WooCommerce leg — fetches a client's published, in-stock WooCommerce products and feeds them to the Generic KB Ingestion Core, same shape as the Shopify leg.

**TRIGGER:** Execute Workflow Trigger (Define Below): `client_id`.

**FLOW:** `Resolve WooCommerce Credential (UTIL-006)` (category `ecommerce`, same fix as the Shopify leg — UTIL-006 gracefully no-ops its refresh step for WooCommerce since `refresh_token_secret_id` is absent, so reusing it here needed no special-casing) → `Credential Available?` → false: throws; true: `Read Consumer Secret` (`read_credential_secret` RPC on `secondary_secret_id` — UTIL-006 only decrypts the PRIMARY secret (Consumer Key), the Consumer Secret is read separately) → `Build Basic Auth Header` (`Buffer.from(key+':'+secret).toString('base64')`) → `Fetch WooCommerce Products` (REST `/wp-json/wc/v3/products`, `per_page=100&status=publish`, `fullResponse:true` so the response's `X-WP-TotalPages` header and actual status/body shape can both be checked) → `Normalize Products` (validates `statusCode` is 2xx AND `body` is a real array *before* trusting it — see the live proof below — then filters to `stock_status` in/`onbackorder`, builds D26-shaped content: name/description/SKU/category/price/attributes) → `Call Generic Ingestion Core`.

**Same disclosed single-page shortcut as Shopify (D28 sibling):** `per_page=100`; `X-WP-TotalPages>1` sets `read_complete:false`, same graceful-degrade contract.

**Same live-found `category` bug as the Shopify leg, same fix** (see that entry — not re-explained here).

**LAST VERIFIED, 2026-09-02 — live, and this is exactly the disclosed risk from Card3's `/plan-eng-review` materializing for real, not a hypothetical (execution 70527 → sub-execution 70538):** `Wiki/credentials/woocommerce.md` had already disclosed `zenny-woocom.free.je` (Client A's only real WooCommerce test store, a free-tier host) as known-flaky, having "returned non-JSON responses to real API calls" before. This pass's live call reproduced exactly that: `Fetch WooCommerce Products` got back `"Response body is not valid JSON"` (n8n's own parse-failure shape, not a `{statusCode,headers,body}` object). `Normalize Products`' explicit shape validation (Codex's catch, folded into the build) correctly treated this as `read_complete:false, records:[]` rather than crashing or silently upserting garbage — the Generic Ingestion Core correctly no-op'd (zero vectors written, zero orphan cleanup attempted, confirmed via `describe-index-stats` showing no `baa673b5-...` namespace at all) and (post-fix, see the core's own entry) `sync_status` now honestly reports `status:'failed'` for this run rather than a misleading `'success'`.

**Real, disclosed gap, not fixed this pass — needs a working test store, not more code:** the WooCommerce ingestion *mechanism* is built and verified to fail safely against a real broken store, but a genuine successful WooCommerce sync has never been proven end-to-end, because no working WooCommerce test store exists in this project (same gap the Wiki page already flagged before this card started). Building around this with a second, throwaway test store was considered out of scope for this card; tracked in `TODOS.md`.
...
**BC-076 follow-up (2026-08-31, same day):** `commerce_ecom_agent_system` (`tpl_commerce` + `client_test_002`) updated to instruct the LLM to call `Search_business_kb` for hours/policy/product questions instead of refusing outright — live-verified the LLM now correctly calls the tool.

**BC-076-Card1 (2026-08-31, same day, later pass) — FIXED:** `Search_business_kb` rebuilt as an `httpRequestTool` calling the KB tool's new real Webhook (see the Search Business KB Tool entry above for full detail) instead of the broken `toolWorkflow` node. `client_id` is now a static reference to `Commerce-Ecom Node Trigger`'s own `client_id` — never LLM-supplied. Live-verified end to end: a real seeded fact returned exact-match for this client, and a cross-tenant question correctly did NOT leak a different client's seeded fact.
...
**FIXED, cross-cutting, severe (found+fixed during BC-076 verification, 2026-08-31) — same `Memory Cold?`/`Get Recent History (RPC)` cold-path bugs documented in full under the Commerce-Ecom Node entry above, identical root cause, identical fix (`rightValue: true`; new `Has History?` gate).** Every brand-new customer's first message to this Agent had the same silent-failure exposure since BC-074 shipped. **Spot-checked with a real cold, zero-history conversation this session (follow-up pass, 2026-08-31 same day)** — a genuinely first-time customer's message ran the full chain end to end (real Agent LLM call, real coherent response, no crash) — the fix holds here too. Synthetic conversation cleaned up after.

**BC-076 (2026-08-31):** new `Search_business_kb` tool added alongside the 4 existing tools. `appointment_agent_system` also got the same BC-076 follow-up prompt update (instructs the LLM to use the KB tool).

**BC-076-Card1 (2026-08-31, later pass) — FIXED:** `Search_business_kb` rebuilt as an `httpRequestTool` (same fix as Commerce-Ecom, see above), `client_id` static via `Appointment Node Trigger`. Live-verified independently, not just carried over from Commerce-Ecom's proof: a real seeded fact (cancellation policy) returned exact-match for this client.
...
**FIXED, cross-cutting, severe (found+fixed during BC-076 verification, 2026-08-31) — same `Memory Cold?`/`Get Recent History (RPC)` cold-path bugs documented in full under the Commerce-Ecom Node entry above, identical root cause, identical fix.** Every brand-new customer's first message to this Agent had the same silent-failure exposure since BC-075 shipped. **Spot-checked with a real cold, zero-history conversation this session (follow-up pass, 2026-08-31 same day)** — ran the full chain end to end (real Agent LLM call, real coherent response, no crash); the response even correctly referenced "the business knowledge base" when it had no answer, suggesting the LLM did attempt the KB tool per the new prompt instruction. Fix holds here too. Synthetic conversation cleaned up after.

**BC-076 (2026-08-31):** new `Search_business_kb` tool added alongside the 3 existing tools. `consultation_agent_system` also got the same BC-076 follow-up prompt update (instructs the LLM to use the KB tool).

**BC-076-Card1 (2026-08-31, later pass) — FIXED:** `Search_business_kb` rebuilt as an `httpRequestTool` (same fix as Commerce-Ecom/Appointment, see above), `client_id` static via `Consultation Node Trigger`. Live-verified: tool call succeeds and gracefully falls back to the "no content yet" message for this client (no fact seeded for Consultation's test client this pass — a real spot-check of the mechanism, not the grounded-content bar Commerce-Ecom/Appointment were held to).
