...
- [infra/convocore-agent-provisioning.md](infra/convocore-agent-provisioning.md) — BC-058: 2 doc/reality gaps resolved (agent naming convention already decided elsewhere; `convocore_agent_map` already built, not "pending" as the Adapter Spec still says), plus a real-but-unconfirmed `agent_prompts` table found that may already scaffold the "Template Dashboard."
...
- `05_Platform_Builds/Convocore/Convocore_Agent_Intake_Checklist_v1.md` — BC-058's master intake checklist (business/archetype ID, agent config, backend provisioning, integration credentials), AUTO/ASK split, doubles as onboarding-manual v1's seed.
