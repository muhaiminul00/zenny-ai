...
**BC-076-Card2b (2026-09-01) — generalized dispatch, not another special case:** this workflow now polls BOTH `source_type='notion'` and `source_type='google_sheets'` due rows (`?select=client_id,source_type,source_ref,source_config&source_ref=not.is.null&source_type=in.(notion,google_sheets)`) and routes each row via a new `Route by Source Type` Switch node — `notion` → `Call INT-012 Sync Notion KB` (unchanged), `google_sheets` → new `Call Sheets KB Ingestion` node calling the new Sheets ingestion sub-workflow (see entry below). **Not yet published** — blocked on the same Credential Gate as the Sheets workflow itself (publishing this workflow requires its referenced sub-workflow to be published first, and n8n's own publish check correctly refused). The pre-existing Notion-only fix above IS live; this dispatcher generalization sits safely in draft until the human provides the Google service-account credential and both workflows can be published together.
...
**n8n ID:** `KR0kHvk3kJRThrX5` · **NOT published** — blocked on Credential Gate (see below), draft validated (13 nodes, structurally sound)
...
**Credential Gate (Standing Rule) — real, not a workaround target:** `Read Sheet Rows` needs a `googleApi` (Google Service Account) n8n credential, built from a real GCP service account's key JSON. **Live-verified this session that the auth mode itself is real** (`n8n-nodes-base.googleSheets` v4.7's type definition confirms `authentication: 'serviceAccount' | 'oAuth2'` natively — the fifth pass had only verified the read *operation* exists, never the auth *mode*; this closes that gap with evidence, not assumption). No credential of this type exists in this n8n instance yet (`list_credentials({type:'googleApi'})` → 0 results). **What's needed from the human:** (1) create a GCP service account, enable the Sheets API for its project, generate a key JSON; (2) create the n8n credential (type "Google Service Account API") from that key; (3) share a real test client's target Sheet with the service account's email (Viewer). Once done, tell Execute to resume — attach the credential, publish this workflow and `SCH-004`, then run the full 10-step live verification plan from `docs/designs/zenny-launch-blueprint.md`'s seventh-pass spec.

**REAL DEPENDENCIES:** `zenny-pinecone-api` (attached), `openrouter-zm` (attached), `zenny-vault-suparbase` (attached) — all pre-existing, no new credential needed for these three. `googleApi` is the one new credential this card needs (Credential Gate above).

**REAL SCHEMA CHANGE (BC-076-Card2b):** `control.client_kb_source` gained a `source_config jsonb` column (D18) — holds `key_column`/`whitelist_columns` per source row, generalizing to whatever future legs (Card 3) need without further overloading `source_ref`. `upsert_client_kb_source` RPC extended with a 5th `p_source_config` parameter — **applied the project's own logged pitfall correctly this time:** the old 4-arg signature was explicitly `DROP`ped before `CREATE OR REPLACE`, and the resulting ACL was checked and corrected (the schema-level default grant had re-added `anon` EXECUTE on the new signature — revoked, leaving only `authenticated`/`service_role`, matching the original's intent).

**NOT YET VERIFIED (blocked on the Credential Gate above, not skipped):** any live execution — reading a real sheet, the delete-then-reinsert proof, the re-key/absence proof, the 403-vs-404 error-status proof, or the end-to-end Agent retrieval proof. All specced in the Build Card, none run yet.
