...
**BC-076 follow-up (2026-08-31, same day):** `commerce_ecom_agent_system` (`tpl_commerce` + `client_test_002`) updated to instruct the LLM to call `Search_business_kb` for hours/policy/product questions instead of refusing outright — live-verified the LLM now correctly calls the tool. **But the tool call itself is broken** (`client_id` resolves to `null` inside the `toolWorkflow` ai_tool boundary — severe, unfixed, full detail under the Search Business KB Tool entry above) so the customer still gets the same "I don't have that information" fallback as before, just via a different, now-functioning code path. Also spot-verified this session: `Build System Prompt`'s substitution logic extended to also inject a literal `[[client_id]]` placeholder (harmless, part of the attempted fix, kept). Synthetic conversations cleaned up after.
...
**BC-076 (2026-08-31):** new `Search_business_kb` tool (`toolWorkflow` → `uZdHEI8tQ1qeeHzt`) added alongside the 4 existing tools. Wired, not live-verified against a real conversation this session (see Commerce-Ecom Node entry for the tool's own proof and the flagged system-prompt gap, which applies here identically). `appointment_agent_system` also got the same BC-076 follow-up prompt update (instructs the LLM to use the KB tool) — but this workflow's own `Search_business_kb` node was left in its original (also broken, `client_id: null`) state, not updated with the Commerce-Ecom experiment; needs the same real fix once one is found.
...
**BC-076 (2026-08-31):** new `Search_business_kb` tool (`toolWorkflow` → `uZdHEI8tQ1qeeHzt`) added alongside the 3 existing tools. Wired, not live-verified against a real conversation this session (see Commerce-Ecom Node entry for the tool's own proof and the flagged system-prompt gap, which applies here identically). `consultation_agent_system` also got the same BC-076 follow-up prompt update (instructs the LLM to use the KB tool) — but this workflow's own `Search_business_kb` node was left in its original (also broken, `client_id: null`) state, not updated with the Commerce-Ecom experiment; needs the same real fix once one is found.
