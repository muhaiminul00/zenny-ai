...
- **D20 re-key/absence proof** (editing an already-synced row's key value must leave the OLD vector gone): blocked by the Viewer-only credential above (correctly so) — needs the human to change one row's `Variant SKU` in the live test sheet directly, then tell Execute to resync and check Pinecone.
- **403-vs-404 revoked-access error-status proof**: needs the human to temporarily unshare the sheet from the service account (an action only they can take on their own Google account) so a real permission-revocation error shape can be observed; re-share to restore.
