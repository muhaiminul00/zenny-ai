...
- [infra/convocore-agent-provisioning.md](infra/convocore-agent-provisioning.md) — BC-058/058c: 2 doc/reality gaps resolved (agent naming convention already decided elsewhere; `convocore_agent_map` already built, not "pending" as the Adapter Spec still said — both corrected in the docs). Plus a 3rd finding, unrelated to Convocore: `control.agent_prompts` is Email Manager's (per-client-overridable LLM prompts). BC-062 (2026-08-15): credential-attach block diagnosed and fixed (n8n tooling gap, not a Supabase permission issue); found the control-schema/archetype-keyed design was the wrong shape — the live `email_categories` precedent shows this content belongs per-client-schema (`public`+`tpl_*`+client schemas), same as the rest of the platform. Redesign not yet built — reported, awaiting decision. See [[platform-quirks/n8n-openrouter-direct-llm-pattern]].
...
- [platform-quirks/n8n-openrouter-direct-llm-pattern.md](platform-quirks/n8n-openrouter-direct-llm-pattern.md) — the `chainLlm`+`lmChatOpenRouter`+structured-output pattern for the first direct AI judgment call made from n8n itself (INT-010), and why it was a human decision, not self-resolved. BC-062 (in progress, 2026-08-15): both prompts now draft-wired to `control.agent_prompts` via a new RPC, blocked on a credential the n8n MCP tooling can't attach itself.
