...
**BC-062 UPDATE (2026-08-14/15), IN PROGRESS — NOT YET PUBLISHED:** new `Get Classification Prompt Template` HTTP node inserted between `List Categories` and `Build Classification Prompt`, calling a new `public.get_agent_prompt(p_module, p_prompt_key, p_archetype)` RPC (SECURITY DEFINER, reads `control.agent_prompts`, granted to `service_role`/`authenticated` only — not `PUBLIC`/`anon`, per the BC-052 anon-grant lesson applied proactively). `Build Classification Prompt`'s literal prompt text replaced with the same wording as a `{{categories}}/{{sender_email}}/{{subject}}/{{body}}` template, seeded live as the one default (`module='email_manager', prompt_key='classify_email', archetype=NULL`) row — output is byte-identical to the prior hardcoded prompt (verified by direct string construction, not yet by a live test run). Closes the BC-058c-disclosed `agent_prompts` wiring gap for this workflow. **Blocked on a Credential Gate the MCP tooling itself can't clear:** the new HTTP node's Supabase credential could not be attached via `update_workflow` — the tool silently drops any `credentials` value passed on `addNode` and returns `"HTTP Request nodes were skipped during credential auto-assignment. Their credentials must be configured manually."` The node currently has NO credential attached in the draft. Production publish validation requires a real credential on every credentialed node, so this workflow cannot be tested or published until a human opens this node in the n8n UI and selects the Supabase credential (believed to be `zenny-vault-suparbase`, `guCWYmcVycnfMixw` — matches the project's own Supabase project name and predates the other candidate, `Zenny Dashboard Service Key Role`, chronologically; not independently confirmed since sibling nodes' credential assignments are redacted from every read this session's MCP scope allows). **Active/production version is still the pre-BC-062 one — no live behavior has changed.**
...
**BC-062 UPDATE (2026-08-14/15), IN PROGRESS — NOT YET PUBLISHED:** same pattern as INT-010 (see its BC-062 entry for the full RPC/table detail). New `Get Draft Prompt Template` HTTP node inserted between `Email Found?`'s true branch and `Escalate Category?` (upstream of both the escalate and normal-draft paths, so the template is available to `Build Draft Prompt` regardless of which branch fires). `Build Draft Prompt`'s literal instruction text replaced with a `{{category}}/{{grounding}}/{{sender}}/{{subject}}/{{body}}` template read from the same `agent_prompts` row seeded for this workflow (`prompt_key='draft_email'`). Output byte-identical to the prior hardcoded prompt (verified by construction). **Same Credential Gate block as INT-010** — the new node's Supabase credential could not be attached via MCP tooling, draft is unpublished, live/active behavior unchanged.
