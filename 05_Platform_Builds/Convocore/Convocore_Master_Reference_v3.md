...
> 📌 **This section describes the single-prompt / Prompt-tab agent path.** Per §6's standard-path note, our builds use Canvas exclusively — including single-node flows for simple agents. The concepts below (mandatory template variables, the 5-step framework, formatting-by-model-family) are **still fully applicable** — they just get applied inside a Canvas node's Instructions (§6.3) and/or the Canvas Global Prompt (§6.4) instead of a standalone Prompt tab. Read this section for the underlying prompt-writing theory; read §6.3–6.4 for where that theory actually gets typed in for our builds.
...
> ⚠️ **Standard-path note:** earlier drafts of this document treated Canvas and the single-prompt "Prompt tab" (§2.2–2.4) as two co-equal, optional paths. As of this revision, **Canvas is our standard build path** — every agent we build goes through Canvas, even simple ones (a simple agent is just a Canvas flow with one node, which is exactly what Turbo Mode, §6.11, is optimized for). §2.2–2.4 remain in this document as background/reference — the underlying concepts (system prompt structure, `{kb_context}`/`{about_context}`, the 5-step prompt framework) are still fully valid — but the Prompt-tab-only workflow they describe is not how we build. Where Canvas has its own equivalent of a Prompt-tab concept (Global Prompt, below), use that instead.

### 6.1 Why Canvas exists

A single system prompt + tool list breaks down for complex agents — e.g. a scheduling agent juggling 10+ tools across many scenarios simultaneously. Canvas decomposes one giant prompt into a **flow graph**: small, focused prompts/toolsets per node, connected by routing that is itself AI-driven (§6.5), not just conditional logic.

A single-node Canvas flow is functionally equivalent to the old single-prompt agent — so "keep it simple" and "use Canvas" are not in tension; simplicity now means *one node*, not *no Canvas*.

### 6.2 Node types

| Node type | Behavior |
|---|---|
| **Start Node** | Auto-created, exactly one per flow, entry point. Runs once per session — handles the opening greeting/framing, then the Router LLM moves the user out to whichever node actually matches their intent. It does **not** re-run unless the conversation fully resets. |
| **Default Node** | Standard operation/decision point — the main building block. Has visible input/output connection points ("wired") — reachable only via an explicit edge from another node, in a fixed sequence. |
| **Global Node** | **Not a separate node type** — it's a Default Node with the **"Global Node" toggle** switched on (confirmed directly from the dashboard: global and default nodes are identical except for this one toggle). Toggling it on **removes the node's input wiring** — a Global Node has no fixed entry point and is instead reachable from *anywhere in the flow, at any turn*, because the Router LLM treats it as "always listening" and checks its description against user intent on every single message, not just when the flow's wiring leads there. |
| **End Node** | Marks a logical conversation/call end. |
| **Condition Node** | A reusable, named condition block with a single **Edge Condition** text field ("Describe when the router of the source node should route to this node"), referenced by multiple edges instead of writing the same condition inline on each one — preferred for maintainability. |

💡 **Global vs. Normal node routing, in plain terms** (paraphrased from Convocore's own support explanation): think of the Router LLM as a receptionist. If a node is **Global**, the receptionist knows that door is always unlocked and can send a visitor there any time their request matches. If a node is **Normal (wired)**, the receptionist only lets a visitor through Door B after they've walked through Door A first — it's locked into the sequence the edges define. Good candidates for Global: "talk to a human," "cancel my order," a general FAQ catch-all — anything a user might ask for at a genuinely unpredictable moment. Good candidates for wired/normal: strict step-by-step sequences where skipping ahead should not be possible (e.g. a "collect email" node wired directly into a "book appointment" tool node).

### 6.3 Per-node configuration

Every node's edit panel has two field groups that are easy to conflate but serve **different consumers** — this is one of the most important distinctions in this whole document:

| Field | Also labeled | Who reads it | Purpose |
|---|---|---|---|
| **Edit Instructions** | "Description" (same box — Overview tab) | The **LLM generating this node's actual response** | The node's own scoped prompt/script — what this node says and does once the user is in it. Equivalent to a mini system-prompt (Canvas' version of what §2.2 describes for single-prompt agents). |
| **"Defines what this node does"** | Sits in a separate box directly under Edit Instructions | The **Router LLM** (§6.5) | A routing-trigger description — tells the router *when* a user's message should land on this node. This is not shown to the end user and does not shape the node's response tone; it exists purely to help the Router LLM make a correct routing decision. |

⚠️ Both boxes are easy to blur together since they sit adjacent in the same panel — but they have entirely different audiences (one is read by the response-generating LLM, the other by the routing LLM) and should be written with that distinction in mind: Instructions can be conversational and detailed; the routing-description field should be a crisp, unambiguous statement of *intent match* ("Use this node when the user wants to book an appointment or check calendar availability" — not "Greet the user and ask what time works for them," which belongs in Instructions instead).

Beyond these two prompt fields, each node also has:

- **LLM Configuration** — this node's own model choice (see §6.6 — every node can run a *different* model from every other node, independent of both the flow's default and the Router LLM), temperature, max tokens
- **Tools** — which tools this specific node can call
- **Knowledge Base** — enable KB retrieval for this node, set chunk count, KB filter tags
- **Router Configuration** — rewind level, auto-rerouter toggle (§6.9), and (on the Start Node specifically) the overall Router LLM model choice

### 6.4 Global Prompt (Canvas-level) — the Prompt-tab replacement

Distinct from **both** other "global" concepts in this document — see the disambiguation table update in §17. Found under the node's **gear icon → Advanced Settings** panel (a flow-wide settings panel, not scoped to any one node).

**What it is:** a block of instructions that is **appended before every single node's own Instructions**, on every turn, regardless of which node the conversation is currently in. This is Canvas' direct equivalent of what a single-prompt agent's System Prompt does, and is confirmed (via Convocore's own support assistant) to be conceptually equivalent to Voiceflow's **Global Prompt + Global Instructions combined** — with one deliberate difference: it does **not** carry Voiceflow-style routing logic, because in Convocore routing is a separate, dedicated mechanism (§6.5), not something baked into the master prompt.

**What belongs here (per Convocore's own guidance):**
- Agent identity, tone of voice, persona — "You are a helpful assistant for Acme Corp"
- Hard boundaries/rules that must apply no matter which node the user is in — "Never offer discounts," "Never discuss competitor pricing"
- Anything that should be true of the agent's behavior universally

**What does NOT belong here** — keep it out and put it in the individual node's Instructions instead:
- Task-specific steps ("If the user wants a refund, ask for their order number") — this is node-specific behavior, not universal identity
- Routing hints — while you technically *can* write routing guidance into the Global Prompt, Convocore's own guidance is that this isn't the primary mechanism and shouldn't be relied on; the Router LLM reads node descriptions instead (§6.5), and the Global Prompt's job is just to keep the agent "in character" while that routing happens, not to drive it

**Best practice (directly from Convocore's support guidance):** keep the Global Prompt focused on universal rules only; push task-specific instructions down into individual node prompts. This keeps the agent focused and measurably reduces hallucination compared to one giant do-everything prompt.

This same Advanced Settings panel also holds: **Global Variables** (data available to every node — same concept as §6.9's variable "Global" toggle, just also settable from this central panel), **Global Tools** (tools available to every node — ⚠️ the dashboard itself warns that adding tools here "might make the AI hallucinate more," and recommends assigning tools to specific nodes instead wherever possible), **Enable Nodes** (the on/off switch for the whole multi-node system — disabling it collapses the agent back to old single-prompt behavior), **UI Engine** and **Fillers on Tool Usage** toggles (see §5 for UI Engine itself; Fillers make the agent say something like "one moment…" / "let me check that" while a tool call is in flight, rather than going silent), and **Start Call Phrases** (voice-specific — custom trigger phrases for starting a call).

### 6.5 The Router LLM — how routing actually works

This is the mechanism that decides, on every turn, which node should handle the user's next message. It is **not** the same thing as the LLM that generates a node's actual reply (§6.6) — they are two separate model instances doing two separate jobs, and each is independently configurable.

**How it decides (confirmed, in plain terms):** on each incoming user message, the Router LLM reads the message together with every reachable candidate node's routing-trigger description (§6.3's second box) plus any edge/condition-node logic (§6.7), and picks the best-matching node **by reasoning**, not by evaluating a strict boolean expression. This is a deliberate design choice: Convocore's own framing is that older flow-builder platforms require you to hand-build and train explicit intent blocks with dozens of trigger phrases, whereas here you write one plain-English description per node and the Router LLM's language understanding handles synonyms, typos, and natural phrasing on its own.

**Two blended inputs, not two separate modes:** it's more accurate to say the Router LLM has two *sources of signal* it weighs together, rather than the flow being locked into "either condition-based or LLM-based" per node:
1. **Structural/conditional signal** — edge conditions and Condition Nodes (§6.7) constrain *which nodes are even reachable* from the current position
2. **Reasoning signal** — among the reachable candidates (plus any always-reachable Global Nodes), the Router LLM judges which one's description best matches the user's actual intent

**Configuring the Router LLM:**
- **Model choice** is fully configurable — the dashboard offers roughly 30 model options for this role specifically (separate from the ~46 options available for node response-generation, §6.6). Convocore's strong recommendation is **Gemini 2.5 Flash** as the default: "This LLM will be used to choose which node should be used based on the conditions, nodes' descriptions, etc. This model has to be very fast or there will be noticeable latency with every interaction." Swapping to a slower/larger model here directly adds latency to *every single turn*, not just complex ones — this is different from node-response model choice, where a slower model only costs you time on the turns that actually hit that specific node.
- **Fallback models** — you can configure one or more backup models the Router LLM will fall through to if the primary model is unavailable/down, so a provider outage doesn't take down routing entirely.
- Set once at the flow level (Advanced Settings / Start Node's Router Configuration), not per-node.

**Auto Rerouter** (seen as a toggle in node Router Configuration): when enabled, the LLM will automatically re-route away from the previous node if needed — i.e. it can self-correct a routing decision mid-conversation rather than getting stuck once a user's intent shifts.

### 6.6 Turbo Mode (latency optimization)

When a flow has **only 1 node**, Canvas automatically skips the Router LLM step entirely (Turbo Mode), saving ~100–500ms per interaction that would otherwise go to routing. This is automatic — not a manual toggle — and confirms why a genuinely single-purpose agent should stay a single Canvas node rather than being artificially split: every additional node reintroduces the router's latency cost on every turn, in exchange for the ability to scope tools/prompts/models per scenario. This is the correct mental model for "when do I actually need multiple nodes" — not "Canvas vs. no Canvas" (§6.1's note), but "one node vs. several," weighed against latency.

### 6.7 Connecting nodes — edges & conditions

- Edges connect nodes; hover a node to see connection points, drag to another node
- **Edge conditions**: click an edge → Condition Editor → define logic using variables (`user_input`, etc.) with `AND`/`OR` operators — this is the *structural* signal the Router LLM (§6.5) uses to narrow down reachable candidates
- **Condition Nodes** (preferred over inline edge conditions for anything reused): a dedicated node with one field, **Edge Condition** — "Describe when the router of the source node should route to this node. This condition will be evaluated to determine the flow direction." Define once, reference from multiple edges.
...
### 6.8 Model Configuration parameters (per node)

Every node has its **own** LLM Configuration, independent of every other node's, of the flow's Global settings, and of the Router LLM (§6.5) — a flow can legitimately mix, say, a cheap/fast model on a simple FAQ node with a stronger model on a complex negotiation node.

| Parameter | Range | Effect |
|---|---|---|
| **Model** | ~46 options at time of writing | The model that generates *this node's* actual replies. Independent of the Router LLM's model choice (§6.5). |
...
| **Max Tokens** | — | Response length ceiling — 100 for short summaries, 500 for detailed explanations; keep low (e.g. 250–500) for faster real-time voice agents |
...
There is also a **Default LLM Settings for Nodes/Tools/Routers** section in the flow-wide Advanced Settings panel (§6.4) — this sets the fallback/default model+temperature+max-tokens used when an individual node hasn't overridden them itself, so you don't have to configure every single node from scratch.

⚠️ **Region note on model availability:** at least one model observed in the dashboard (Grok 4.5 - XAI, "Think: high") is explicitly **NA-region only** — selecting it while on an EU workspace silently reroutes at runtime to a lower Grok model (grok-3-fast) rather than erroring. Don't assume every listed model is available in every region; verify against your actual workspace region if a specific model choice matters to you.

### 6.9 Rewind — deeper detail
...
- Mark as **Global** to make accessible from every node (same underlying concept as the Global Variables section inside flow-wide Advanced Settings, §6.4 — two entry points to the same setting)
...
### 6.12 Calling tools inside Canvas (two mechanisms)
...
⚠️ CDN update (found 2026-07-27): live MCP testing returned an embed snippet using https://cdn.convocore.ai/vg_live_build/... — different from the vg-bunny-cdn.b-cdn.net URL below. Convocore appears to have migrated CDNs since this doc was written. Treat cdn.convocore.ai as more current; verify against a freshly-generated snippet (dashboard or MCP's get_website_embed_code) before deploying, rather than trusting either URL blindly.
...
3.  Write the core prompt content using the 5-step framework (§2.2):
    tone/objective → context+boundaries → CoT → few-shot → reinforcement
    → MUST include {kb_context} and {about_context} wherever KB retrieval applies
    → Use Markdown if primary model is GPT-family, XML if Claude-family
    → Split this content across Canvas's two prompt layers (§6.4, §6.3):
      universal identity/rules → Canvas Global Prompt; task-specific
      behavior → individual node Instructions

4.  Build in Canvas (§6) — this is our standard path for every agent,
    not an optional complexity tier:
    → Single-purpose agent → single Canvas node (Turbo Mode, §6.6,
      auto-applies — no router latency cost)
    → Multi-scenario agent → multiple nodes (billing + technical + sales
      handoff as genuinely distinct nodes with different tools/tone)
    → Either way: write each node's routing-trigger description
      (§6.3's "Defines what this node does" field) as a clear intent
      statement — this is what the Router LLM (§6.5) actually reads
      to decide where a message goes, separately from that node's
      Instructions field
    → Set the Router LLM model deliberately (§6.5) — default to
      Gemini 2.5 Flash unless you have a specific reason to change it;
      slower models here add latency to every single turn
...
13. Test (§6.11 for Canvas Test Mode, §4.4 Preview LLM for tools,
...
⚠️ **"Global" has THREE distinct meanings in Convocore — the most collision-prone term in this whole document:**

| "Global ___" | Scope | Set where | Purpose |
|---|---|---|---|
| **Global Prompt** (workspace-level, §2.3) | Across multiple *agents* | Workspace-level Prompts dashboard | Shared instructions assigned to many agents at once, max 10 per agent |
| **Global Prompt** (Canvas-level, §6.4) | Across every node in *one flow* | That flow's Advanced Settings (gear icon) | Appended before every node's Instructions on every turn — this flow's equivalent of a master system prompt |
| **Global Node** (§6.2) | Reachable from anywhere in *one flow* | Per-node toggle on a Default Node | A node with no fixed input wiring — the Router LLM can jump here from any point in the conversation |

When in doubt, ask: *is this about many agents, one flow's master prompt, or one node's reachability?* — that immediately resolves which "global" is meant.

| Term | Meaning A | Meaning B |
|---|---|---|
| **"Prompt"** | **System Prompt** (§2.2) — the core agent behavior definition (Prompt-tab path; see §6's standard-path note for how this maps onto Canvas) | **Initial Prompt** (§2.4) — instruction for generating the first message only, overrides Initial Message |
...
| **"Router" / "Routing"** | **Router LLM** (§6.5) — the dedicated model deciding which node handles a turn, reasoning over descriptions + conditions | **Router Configuration** (per-node panel, §6.3/6.9) — where rewind level and Auto Rerouter live; configures routing *behavior*, doesn't itself do the routing |
| **"Description" (on a node)** | The **Instructions/Description box** (Overview tab, §6.3) — this node's own response-generation prompt | The separate **"Defines what this node does" box** (§6.3) — the routing-trigger text the Router LLM reads; different field, different reader |
...
A safe, isolated environment to validate an agent before it's live, separate from Canvas's own Test Mode (Section 6.11 — Canvas testing is flow-specific; Prototype Testing is whole-agent/widget-level).
...
Simulates realistic customer conversations against your agent and produces a structured quality analysis — distinct from both Prototype Testing (Section 20.1, manual/human-driven) and Canvas Test Mode (Section 6.11, flow-structural).
...
- **Next planned step** (per original direction): cross-map this document's concepts to our own system/runtime terminology once this version is validated as accurate. Phase 2: full API/endpoint reference document (delivered separately as `Convocore_API_Reference_v1.md`).
- **v3** (this version) — architectural correction to §2 and §6, based on direct dashboard screenshots and a transcript with Convocore's own support assistant. Key fixes: (1) established **Canvas as the standard build path** (single-node Canvas for simple agents, not a separate Prompt-tab path) with §2.2 marked as background/reference theory rather than an active workflow; (2) added **§6.4 Canvas-level Global Prompt** — a third, previously undocumented "global" concept, flow-wide and appended before every node's Instructions, conceptually equivalent to Voiceflow's Global Prompt + Global Instructions combined, but without routing logic; (3) added **§6.5 Router LLM** — corrected the routing model from "conditions or LLM reasoning, node-by-node" to "one blended mechanism per turn": structural signal (edge conditions/Condition Nodes) narrows candidates, then a dedicated, independently-configurable Router LLM (~30 model options, Gemini 2.5 Flash recommended default, fallback models supported) reasons over each candidate node's routing-trigger description to pick the best match; (4) clarified **§6.3** that every node has two visually-adjacent but functionally distinct text fields — Instructions (read by the node's own response-generating LLM, independently model-configurable, ~46 options) vs. the routing-trigger description (read only by the Router LLM); (5) corrected **Global Node** (§6.2) to be explicitly "a Default Node with input wiring removed," not a separate node type; (6) expanded §17's terminology table to a 3-way breakdown of "Global" and added Router/Description disambiguation rows. Net effect: §6 changed from "routing is conditions, with an LLM doing turbo-mode node-skipping as an aside" to "routing is Router-LLM-driven reasoning constrained by conditions, and that Router LLM is a first-class configurable component" — a meaningfully different and more accurate model of how Convocore actually decides where a conversation goes.
