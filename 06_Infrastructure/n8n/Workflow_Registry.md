...
**CREDENTIAL GATE, BC-048 update — Pinecone leg resolved, Notion leg newly found broken:** the Pinecone credential (`zenny-pinecone-api`, created BC-048) turned out to be a native `pineconeApi`-typed credential, not the `httpHeaderAuth` type this workflow's sticky assumed — `Upsert To Pinecone` switched from `genericCredentialType`/`httpHeaderAuth` to `predefinedCredentialType`/`pineconeApi`, live-verified working (see INT-010's chain test — real, unauthenticated-error-free Pinecone query). **New, still-open gate:** live-testing this workflow directly hit a real 404 from Notion — `List Child Pages` (credential `zenny-notion-api`, id `xsukueLOwvqr1es2`) cannot see the client's own KB root page, even though the exact personal-access-token value supplied in chat (verified live via raw `curl` against `api.notion.com`) belongs to the "n8n" bot integration that *does* own and can see that page. This means the n8n credential's stored secret does not actually match the token that was supplied — the MCP cannot read or fix a stored credential secret. **Human action needed:** open the `zenny-notion-api` credential in the n8n UI and re-paste the exact Internal Integration Secret token, then this workflow's Notion→Pinecone round trip can be genuinely live-verified.
...
**BC-048 UPDATE (2026-08-13) — genuinely live end to end (Pinecone leg):** triggered for real via INT-010's new chain call against a real inbound test email. Real customer resolution → real "Support" categorization (after the `List Categories` fix, see INT-010) → real `Embed Query (OpenRouter)` call → real `Query Pinecone` call (0 matches — Client A's namespace is still empty since INT-012/Notion hasn't synced yet, see INT-012's Credential Gate) → correctly fell back to `client_config` grounding → real `chainLlm` draft generation → real `update_client_email_draft` write, confirmed via direct SQL (`email_status: 'draft_ready'`, `reply_style: 'generative'`, real non-empty `draft_content` that correctly avoided inventing specific policy details per the fallback prompt's instruction). This is the first genuinely-live, unpinned proof this workflow's Pinecone leg authenticates and works — only the KB-match branch (INT-012's Notion leg) remains unverified live, see INT-012.
