...
human feedback after live-testing the shipped feature: "Add Admin"
shouldn't require picking a client. Went through `/plan-eng-review`
(design doc `docs/designs/admin-provision-client-picker-fix.md`),
Codex outside-voice (13 findings — 1 genuine cross-model tension on
scope, resolved; 5 uncontested corrections folded in; 2 new TODOs
logged; 3 checked and confirmed not applicable to this codebase), then
shipped via Execute.

**Root cause:** `control.dashboard_users.client_id` was `NOT NULL`,
built for client accounts. The Bootstrap card's `create_admin` action
worked around this by requiring an arbitrary "nominal home client" just
to satisfy the constraint — confusing UX, not intentional design.

**Fix:** `client_id` made nullable with a strict CHECK
(`client_user` → must be set; `admin`/`super_admin` → must be NULL).
Migration order: drop `NOT NULL` → backfill existing admin-tier rows to
NULL → add the CHECK (reverse order would fail immediately against
`admin@zenny.internal`'s then-still-set `client_id`, which was
`baa673b5-c51a-4a7b-91f5-a37027f8dca4`, Client A — logged here for
rollback reference). `create_admin`'s Edge Function branch no longer
reads/validates any client field — passes `p_client_id: null` directly.
`AddAdminTab` lost the picker; the 3 provisioning tabs got visually
regrouped (Clients-related vs. "Internal accounts") per the same human
feedback.

**Real, independent bug found and fixed in the same pass** (surfaced by
this fix's own audit of every `dashboard_users.client_id` consumer, not
part of the original complaint): `dashboard_admin_list_clients()`'s
login-email lookup had no role filter, so it could show an admin's
email as a client's "login" under the right (unlucky) timing. Added
`AND du.role = 'client_user'` to the lateral join, regression-verified
against all 6 real clients before/after — unchanged.

**Live-verified end to end:** schema (nullable + CHECK via
`information_schema`/`pg_constraint`), the list-view regression (all 6
real clients), `create_admin` via curl with a real super_admin JWT (no
client field required, `client_id` NULL on the new row), the
`dashboard_provision_user` (client_id, role) pairing held across a real
remap, `map_existing` and the `SUPER_ADMIN_REQUIRED` gate both
regression-checked, and the UI itself via `gstack /browse` against a
local dev server pointed at the live backend (all 3 tabs render and
submit correctly, zero console errors). All synthetic test accounts
(5 total across the curl and browser passes, including one created
during an initially-invalid negative-test attempt) cleaned up after,
zero leftover confirmed live.

**Deliberately deferred, not silently dropped** (cross-model tension
with Codex, human-resolved): a shared `useProvisionForm` hook across
the 3 tabs — logged in `TODOS.md`, not built now (regression risk on 2
already-working tabs with zero test coverage, for a fix whose real bug
was schema-level). Also deferred and logged: explicit role-guard
hardening on 4 client-only RPCs (currently reject admins correctly, just
...
with its own stated design (current-truth dashboard, not an archive).
Approach: rather than verify each of the file's ~44 distinct historical
entries individually against this log (checked first — only 9 of 44
cited `session-*` slugs actually match a `## session-*` header here;
the rest predate that convention or are under different heading text),
the entire pre-prune file was archived verbatim below (see "PROJECT_
STATE.md full pre-prune archive") as a zero-loss safety net, then the
new file was written fresh from what's genuinely current: the active
Phase 14 (SaaS pivot) track only, Phases 0-13 (pre-pivot Convocore
build) reduced to a one-line "paused, archived" pointer. Two doc-debt
items surfaced during the cut (the `appointments` table and SCH-007
being undocumented) were still genuinely relevant to the current track
(not just pre-pivot debt) — relocated to `TODOS.md`'s new
"Documentation debt" section rather than archived as dead history.
`Wiki/index.md`'s one stale cross-reference into the old file's "Next
Build Card" section (pointing at BC-070/BC-065-069) was corrected to
point at the archive instead. Committed direct-to-`main` per the
trivial-housekeeping exemption (doc-only, no code/workflow riding
along).
...
## PROJECT_STATE.md full pre-prune archive (2026-09-02, BC-079)

BC-079 pruned `PROJECT_STATE.md` from 1774 lines down to a real
current-truth dashboard (its own stated design). Given the file's
scale and that many older entries pre-date a clean per-session
archival-slug convention (verified: only 9 of 44 distinct `session-*`
references cited by the file actually match a `## session-*` header
in this log — the rest are either under different heading text or
genuinely not separately archived), a full per-entry verification
pass was disproportionate to a doc-only housekeeping card. Instead,
the ENTIRE pre-prune file is captured verbatim below as a safety net
— guaranteeing zero content loss regardless of pre-existing overlap
with content already elsewhere in this log. Any genuinely-still-open
item found in the cut content was cross-checked and either kept in
the new `PROJECT_STATE.md`'s Active Blockers section or is preserved
verbatim right here.
...
## Last Updated
2026-09-02 (latest) — by /execute — **`gstack-pilot` v1.6.1 shipped: the mode.json handback deadlock T7.2 surfaced is fixed and released.** Human asked directly for the fix. `hooks/pre-tool-use.js` (in `E:\Programming\gstack-pilot`, the plugin's own source repo) now exempts a write whose target resolves to `mode.json` itself from the branch/marker check — narrow, nothing else exempted. **Process miss self-caught:** the fix was first drafted/tested directly in Commander mode (a git-write-bound task), corrected by handing to Execute before anything was committed — no rework needed. **Live-verified, and the first test attempt was wrong:** a bash/MSYS path-conversion + JSON-escaping bug made every test scenario silently look like "allow" regardless of the real fix; rebuilt as a Node test harness with real Windows-native paths, which confirmed the fix works (mode.json write + mismatched marker now allows) and stays narrow (a different file, same mismatched marker, still denies). Shipped PR #10, squash-merged, tagged/released `v1.6.1`. Manual proportionate review used in place of gstack's full pipeline (Review Army/Codex/slop-scan disproportionate to a 29-line Node-hook diff with no SQL/LLM/shell-injection surface) — one informational P2 noted, not fixed (case-sensitive Windows path comparison, fail-safe direction). **Confirmed live, 2026-09-02 (same day, follow-up):** human ran `/plugin update gstack-pilot@gstack-pilot`; verified via `~/.claude/plugins/installed_plugins.json` — this project's entry now resolves to `installPath`/`version` `1.6.1`. The fix is in effect for this project going forward; no further action needed on it. Full detail: `Wiki/log.md` session-gstack-pilot-v1.6.1-modejson-exemption-shipped, `Wiki/reference/gstack-pilot-plugin.md`. **Next:** resume BC-076's remaining cards (2a/3/4) — see below.

2026-09-02 (prior) — by /execute — **BC-077's remaining T5/T7 closed, both real work, not a formality pass.** T5: `.claude/CLAUDE.md`'s blank "Fill in the specifics" fields filled from facts already on record (State Doc, live-infra list, Build Card format, env convention) — no new decisions. **T7.2 found a real gap:** BC-078 (commit `00d5851`, PR #8) was doc/config-only, zero code/workflow, exactly what root `CLAUDE.md`'s trivial-housekeeping exemption describes — yet it went through a full PR anyway, not direct-to-`main`. The exemption existed on paper but wasn't applied the one real chance it had. This session's own wrap-up is the corrective live example: committed direct-to-`main`, citing the exemption. **T7.1 (Commander self-chains into `office-hours`/`plan-eng-review` unprompted) confirmed present in source, not yet live-triggered** — no genuinely-fresh or prior-scoped-next-step Commander task has occurred since gstack-pilot went live today to exercise it; deliberately not faked with a synthetic test (real gstack telemetry cost, disproportionate to confirming one dispatch branch) — closes naturally on the next real planning task (BC-076 Card 2a/3 are candidates). **Also fixed in passing:** `.claude/hooks/state/preflight-ok` (the pre-flight marker itself) was untracked and ungitignored, recurring as dirty-tree noise every session — added to `.gitignore`. Full detail: `Wiki/log.md` session-bc077-t5-t7-closed, `Wiki/reference/gstack-pilot-plugin.md`. **Next:** T7.1 stays open, not blocking; otherwise BC-077/078 are now fully closed.

2026-09-02 (prior) — by /execute — **Root `CLAUDE.md`'s "## Modes — /commander, /execute, /advisor" section (135 lines) removed by direct human edit, committed `d1ce202` direct-to-`main` per explicit instruction, no PR** — the mode-behavior description is now considered fully redundant with the `gstack-pilot` plugin (BC-077/078). **One dangling reference fixed as part of landing it:** the Branch/PR Workflow section's "over-engineering this document already tells both modes to avoid" pointed at the now-deleted "Neither mode over-engineers" paragraph — reworded to stand alone. **Flagged, not reversed — real content this deletion drops with no surviving duplicate elsewhere in the doc:** the 2026-08-12 incident note behind the mode-state-write-before-infra-touch discipline; the explicit "live infra = n8n/Supabase/VPS/DNS" domain list tied to the safe-gate; the `/clear`-vs-`/compact` reasoning + its `Wiki/platform-quirks/mode-self-invocation-limits.md` pointer; the "Neither mode over-engineers" philosophy statement. **Confirmed still safe:** the Mandatory MCP Verification rule survives as its own separate standing-rule section elsewhere in this file; the actual safe-gate number (3) survives in `.claude/CLAUDE.md`, which is the file gstack-pilot's commands actually read. Full detail: `Wiki/log.md` session-claude-md-modes-section-removed.

2026-09-02 (prior) — by /execute — **BC-078 SHIPPED: two real gaps in BC-077's own aftermath, closed same day.** Human flagged, right after BC-077's handback, that raw role-mode-era ambiguity could cause dual-instruction confusion, plus asked whether Zenny's standing rules (Credential Gate, etc.) were "restricting" gstack. Both real: (1) `.claude/CLAUDE.md`'s gstack-pilot seed block still had its unedited default Live-infra safe-gate number (5), while root `CLAUDE.md`'s own real standing number is 3 — and gstack-pilot's own `commander.md`/`execute.md` explicitly read the override from `.claude/CLAUDE.md`, not root `CLAUDE.md`, so the wrong number would have silently governed. Fixed: `.claude/CLAUDE.md` now says 3. (2) Root `CLAUDE.md`'s "## Modes" section used bare `/commander`/`/execute`/`/advisor` throughout, harmless when only one mode-plugin existed but now ambiguous since `role-modes` stays installed (disabled, not uninstalled) alongside `gstack-pilot` — literally demonstrated live: the human's own next message invoked `/role-modes:commander`, which still resolved and ran (expected, one-session lag before `enabledPlugins:false` takes effect — not a bug). Fixed: a clarifying line added stating gstack-pilot is the live plugin and what the bare references mean. **Confirmed not a real issue, explained rather than changed:** Zenny's own Standing Rules (Credential Gate, Document Resolution Authority, etc.) are not restrictions on gstack — gstack-pilot's commands are explicitly built to defer to them, confirmed by source-read during BC-077's own design pass, not assumed. No `/plan-eng-review` chain — mechanical reconciliation of two already-on-record numbers, not a new architecture decision. Full detail: `Wiki/log.md` session-bc078-safe-gate-reconcile-shipped.

2026-09-02 (prior) — by /execute — **BC-077 SHIPPED: Zenny migrated from plain `role-modes` to `gstack-pilot` (T1-T4, T6, T8 complete; T5, T7's next-session live-verify still pending).** Human's live complaint (gstack "isn't confident, isn't chaining itself") plus a confirmed grep-fact (only 6 of 55 gstack skills ever invoked in Zenny's build history, all from mandatory standing rules) both traced to the same root cause: Zenny never adopted `gstack-pilot`, the already-built, `zm-brain`-proven successor plugin. Routed through `/plan-eng-review` (Commander→gstack bridge, one of its last uses before being retired by this same card) — Codex outside-voice pass reversed D1 (kept Zenny's trivial-housekeeping PR exemption, via a new project-declarable override point) and held D2 (routing-table merge, same PR). **Built:** `.claude/settings.json` — `role-modes@role-modes` flipped to `false` (disabled, not uninstalled — the switch mechanism is flipping it back), `gstack-pilot@gstack-pilot` stays `true`; root `CLAUDE.md` — "Gstack planning bridge" retired to a one-line superseded pointer (BC-072 history kept intact), Tool Routing Table merged with gstack's own auto-injected "## Skill routing" section (one reference now, not two); 3 Wiki reference pages updated (`role-modes-plugin.md`, `gstack-pilot-plugin.md`, `gstack-skill-playbook.md`) with supersession banners, all historical narrative kept unedited; repo-wide grep sweep found no other live (non-historical) `role-modes` references. **Cross-repo:** `gstack-pilot` itself patched (T3) — `execute.md`'s PR-first-always default is now a narrow, project-declarable opt-in override, mirroring its existing Live-infra threshold-override pattern; `/review` caught 5 stale "no exemption" assertions the new opt-in orphaned (session-start.js, session-end.js, init.md, VERIFICATION_CHECKLIST.md, README.md), all fixed same PR; shipped as `gstack-pilot` v1.6.0 (PR #9, merged, tagged, released). **Not built this pass, by design:** T5 (filling gstack-pilot's seeded `State Doc:` line) — the seed block only exists after gstack-pilot's own SessionStart hook fires post-T1, so it's the first task of the *next* session, not this one. **Flagged, not fixed (out of scope):** `gstack-pilot`'s own `scripts/check-init-sync.js` fails on a clean `main` checkout — confirmed pre-existing via a worktree check against `main` before this card touched anything, not a regression; needs its own investigation. Full detail: `docs/designs/zenny-gstack-pilot-migration.md`, `Wiki/log.md` session-bc077-gstack-pilot-migration-shipped. **Next session's first action:** T5, then live-verify the Verification-plan table (does Commander actually self-chain into `office-hours`/`plan-eng-review` without a human naming the skill; does a trivial wrap-up correctly take Zenny's declared exemption instead of forcing a full PR).

2026-09-01 (prior) — by /execute — **BC-076-Card2c SHIPPED: D20's rename-safety gap is CLOSED, published, live-verified — the two real orphan vectors (`DPB`/`TMB02`) left as evidence since Card2b are actually gone now, independently re-confirmed.** Routed through gstack `/plan-eng-review` per the Commander→gstack→Execute bridge (not freehanded): 6 architecture/code-quality decisions resolved interactively, then the default-on outside-voice (Codex) pass found 16 more findings, 2 of them genuine cross-model tensions surfaced back to the human — most importantly, a missing guard against an incomplete Sheets read mass-deleting real, current KB content, which the first draft had no defense against at all. Fixed as D29 (a read-completeness gate); implementing it also surfaced a second, pre-existing latent gap (n8n's own zero-item-safety default meant a "successful" zero-row read would have silently skipped the whole rest of the workflow with no error, no status write) — closed together. **Built:** 4 new nodes in `KR0kHvk3kJRThrX5` (read-completeness gate, Pinecone `/vectors/list` diff, orphan-key computation, exact-ID delete-in-batches), a shared `id_prefix` builder (DRY fix), duplicate-key exclusion. Live-verified the Pinecone list/pagination mechanics against the real index BEFORE building on them, not assumed. **Live proof, not simulated:** a real production SCH-004 sweep correctly identified exactly `DPB`/`TMB02` as orphaned among 32 listed vectors — zero false positives against the other 14 valid keys (including the renamed replacements `DPBP`/`TMB02002`) — and deleted them; independently re-verified via a fresh, separate Pinecone list call afterward (32→28 vectors, the right 4 gone). **Also formally supersedes D16** (cross-leg row deletion, previously deferred to a future Card 3) — indistinguishable from a rename given key-only row identity, so one mechanism now closes both. **Disclosed scope trim:** the plan's cross-run `consecutive_orphan_step_failures` counter wasn't built (needs a new Supabase read for marginal value) — folded into a `TODOS.md` follow-up along with a periodic dormant-source orphan audit. Full detail: `06_Infrastructure/n8n/Workflow_Registry.md`'s Sheets Ingestion entry (D24-D32), `Wiki/log.md` session-bc076-card2c-d20-rename-safety-fix-shipped. **Housekeeping flag, not this card's scope:** this file is 1700+ lines, well past the ~150-line target this file's own header states — worth a real prune/archive pass (move aged "(prior)" entries' substance into Wiki/log.md, which already has most of it) next time Commander runs a Wiki health-check.

2026-09-01 (prior) — by /execute — **BC-076-Card2b SHIPPED: Credential Gate cleared, both workflows published, live-verified end to end with real data — 4 real bugs found and fixed along the way, none of them credential-related.** Human supplied the service account, n8n `googleApi` credential ("Google Service Account API", `ERpU1vbQ14oSyPFD`), and a real raw Shopify product-catalogue export as the test sheet. **Key-column trap caught before shipping:** the raw export's `Handle` column repeats across every variant row of a product — picking it as D14's key column (the natural-looking choice) would have made D20's delete-then-reinsert silently clobber every variant but the last; confirmed `Variant SKU` (genuinely unique) with the human instead of guessing. **Human also asked for real image-based product search + a recommendation carousel** (upload a photo, agent finds matching products) — correctly identified as new scope outside Card2b (needs its own vision-embedding/index design), deferred as a future Build Card rather than improvised; image URL/alt-text ingested as plain searchable text for now, which is genuinely different from visual search. **4 real bugs found via live execution, none caught by `validate_workflow`:** (1) `Has Valid Rows?`'s branch logic was inverted — real valid rows would have skipped the entire embed/upsert pipeline and written a fake "success" status with zero vectors ever created; (2) same node, a strict-boolean-type crash from a leftover placeholder `rightValue`; (3) `Chunk Content`'s Code-node mode (`runOnceForEachItem`) was incompatible with returning an array of chunks, corrupting downstream items; (4) the one that mattered most — after fixing 1-3, a live run reported clean "success" (14/14 rows) while EVERY upsert had actually failed (0-dimension vector), because a 404 from `Delete Existing Row Vectors` (expected — nothing to delete on a first sync) wiped the row's content before `Chunk Content` read it, and `onError: continueRegularOutput` silently swallowed the resulting embed/upsert failures too. Fixed by sourcing row data from the last Code node instead of the fallible HTTP node's output. **Live-verified independently, not just trusted from the workflow's own status write:** 28 real vectors confirmed via Pinecone `describe-index-stats`; `SCH-004`'s generalized dispatcher correctly routed both the `notion` (6 real clients) and `google_sheets` legs in the same live sweep, 3 times over while debugging; `Search_business_kb`'s real webhook returned the exact seeded product content for a live query. **Revoked-access 403 proof CLOSED, 2026-09-01 (same day, follow-up):** human temporarily unshared the test Sheet from the service account; a live SCH-004 sweep (execution 69959) hit a clean `403 PERMISSION_DENIED` at `Read Sheet Rows` — no crash, no silent no-op, no false-success write to `sync_status`. **New finding flagged, not fixed (out of Card2b's scope):** SCH-004's `Call Sheets KB Ingestion` node has no `continueOnFail`, so one client's Sheets failure marks the whole SCH-004 parent execution `status: error` even though the parallel Notion leg succeeded — worth a small follow-up once more than one client is on the Sheets leg.

**D20 + D17 final proofs run, 2026-09-01 — real issues found, NOT papered over; D17 now closed, D20 still open by design.** Human re-shared access, renamed two live rows (`DPB`→`DPBP`, `TMB02`→`TMB02002`) and added one blank-key row, then a live resync ran (execution 69967). **D20 re-key proof FAILED — a genuine design gap, not a code bug:** confirmed via a disposable diagnostic workflow fetching exact vector IDs from Pinecone directly that the OLD vectors (`DPB`/`TMB02`) are still present alongside the NEW ones — a key rename leaves stale content permanently orphaned, because the delete-then-reinsert mechanism only ever knows a row's *current* key, never a prior one. **Human decided:** track previously-synced row keys and diff-delete anything missing (handles rename/delete/reorder). This is real new architecture (persistence + diff logic + real edge cases) — per the Commander→gstack→Execute planning bridge it needs its own `/plan-eng-review` pass before a Build Card, so it's **NOT built yet**, tracked as a follow-up card. The `DPB`/`TMB02` orphan vectors are deliberately left in Pinecone as real evidence until that card lands. **D17 blank-key proof: CLOSED.** Structurally passed (row correctly excluded, no corruption) but exposed bug #5 — `sync_status` reported `"success"/failed_count:0` while naming a failed row in the same payload. Root-caused (a stale per-row snapshot read instead of an exact derivation), fixed (`failed_count = total_rows - synced_count`), published, and re-verified live: re-ran the sweep and independently queried `control.client_kb_source` directly — `{"status":"partial","synced_count":14,"total_rows":15,"failed_count":1,...}`, now correct. Full detail: `06_Infrastructure/n8n/Workflow_Registry.md`'s Sheets Ingestion entry. **Card2b's DoD is not fully closed — D20's fix is a separate, not-yet-planned Build Card — but nothing currently blocks opening the PR for what IS built and verified; that's the human's call next.**

2026-09-01 (prior) — by /execute — **BC-076-Card2b BLOCKED on Credential Gate, real progress locked in first.** Human update: Google OAuth branding verification already approved ("Zenny" on consent screen), only the explainer video remains before restricted+sensitive scope submission — D11's service-account design was chosen specifically so Card 2b never depends on that timeline, so this doesn't change Card 2b's build, just Part 1's launch-gate tracking. **Found and fixed a real live incident before it happened:** `SCH-004`'s *published* version was still querying `client_kb_source.notion_page_id`, a column renamed away to `source_ref` during BC-076's first-slice migration — its next scheduled run (tonight) would have 400'd, silently breaking the nightly Notion KB sync for every connected client. A prior session's registry note had wrongly claimed this was "updated in the same migration" without ever confirming the fix was actually published (classic draft-vs-published quirk, compounded by an unverified claim). Fixed and published for real. **Built, structurally validated, NOT yet live-tested:** the new `Zenny Runtime - Sheets KB Ingestion (BC-076-Card2b)` workflow (13 nodes) and `SCH-004`'s generalized dispatcher (routes `notion`→existing sync, `google_sheets`→the new leg via a Switch node, not another special case) — both correct per D14-D23's seventh-pass eng review, both blocked from publishing by n8n's own dependency check until the Google service-account credential exists. **Migration applied:** `control.client_kb_source` gained `source_config jsonb` (D18); `upsert_client_kb_source` RPC extended to 5 args, with the project's own logged CREATE-OR-REPLACE-signature-change pitfall applied correctly this time (old signature explicitly dropped, ACL re-verified, the schema-level `anon` grant leak caught and revoked). **Real design corrections made during the build, not just planning** (Pinecone's serverless tier doesn't clearly support delete-by-metadata-filter per its own docs, so D20's cleanup uses delete-by-ID against deterministic candidate IDs instead; `content_hash`/`source_ref_hash` dropped in favor of simpler mechanisms with no unverified crypto-module dependency; D17's "blank cell" failure trigger narrowed to the key column specifically). Full detail: `06_Infrastructure/n8n/Workflow_Registry.md`'s SCH-004 + new Sheets Ingestion entries. **Credential Gate, exact ask:** human needs to (1) create a GCP service account + enable the Sheets API + generate a key JSON, (2) create the matching n8n credential, (3) share a real test client's Sheet with the service account. **Next:** once that's done, Execute resumes — attach credential, publish both workflows, run the full 10-step live verification plan already specced in `docs/designs/zenny-launch-blueprint.md`'s seventh-pass section.

2026-08-31 (prior) — by /execute — **BC-076-Card1 SHIPPED: the severe `Search_business_kb` client_id-null bug is FIXED and live-verified — the KB tool actually works now.** Restructured (not patched): the tool's sub-workflow now has a real Webhook trigger (same shape as WF-002/013/015) instead of the broken `toolWorkflow`/`executeWorkflowTrigger` mechanism; all 3 Agents now call it via `httpRequestTool` (matching `Check_availability`'s proven pattern) with `client_id` as a static main-chain reference — never LLM-supplied, per D13's tenant-isolation correction. **Live-verified with real content, not assumed:** seeded 2 real facts into 2 different clients' Pinecone namespaces, then proved via 4 real conversations — Commerce-Ecom and Appointment each returned their own client's exact seeded fact; the same Commerce-Ecom client asking an unrelated question did NOT leak the other client's fact (real cross-tenant isolation proof); Consultation's tool call succeeds with a graceful fallback. All cleaned up after (test vectors deleted, temp workflows archived, synthetic conversations purged). Full detail: `06_Infrastructure/n8n/Workflow_Registry.md`'s Search Business KB Tool + all 3 Agent entries, `Wiki/log.md` session-bc076-card1-shipped. **Next:** Cards 2a (dashboard OAuth investigation + test-client provisioning) / 2b (Sheets via service account) → Card 3 (remaining ingestion legs) → Card 4 (canary/smoke-test) — each still needs its own `/plan-eng-review` pass before a Build Card, per the sixth-pass sequencing already locked in `docs/designs/zenny-launch-blueprint.md`.
