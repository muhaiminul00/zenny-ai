...
- **D30 — pagination capped** (`maxRequests: 50`, live-verified working correctly against the real index) **and deletes are batched** to Pinecone's documented 1000-ID-per-request limit (not live-tested at that scale — current volume is 4 IDs — but the batching code is in place and correct).
