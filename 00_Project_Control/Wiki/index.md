...
- [credentials/test-fixture-clients.md](credentials/test-fixture-clients.md) — which of the two existing test/demo accounts has which real, live provider connection (Calendar/Gmail/WooCommerce on one, Calendly/Shopify on the other), and why Google/Calendly credentials can't be "backed up" past a real disconnect while Shopify/WooCommerce already survive one (local-only disconnect).
...
- [infra/dashboard-auth-mapping.md](infra/dashboard-auth-mapping.md) — `control.dashboard_users`, the real mapping table (BC-051) that replaced the `app_metadata` stopgap; the new `dashboard_provision_user` RPC for creating dashboard users going forward. **BC-076-Card2a (2026-09-02):** the "not yet built" admin UI gap closed — `role='admin'` + a new Edge Function (`admin-provision-dashboard-user`, this project's first version-controlled Edge Function source) + `/admin/provision` page; admin/client UI fully separated (real bug found live); see the page's own new section for the Codex-review fixes and one live archetype-enum-cast bug caught by the browser click-through. **Admin Provisioning Bootstrap (2026-09-02):** `super_admin` tier, real `create_client`/`create_admin` Edge Function actions, forced password-reset — all live-verified. **Client-picker fix (2026-09-02, same day):** `dashboard_users.client_id` made nullable (strict role-conditional CHECK) so Add Admin no longer needs a "nominal home client"; a real, independent role-filter gap in `dashboard_admin_list_clients()` found and fixed in the same pass.
