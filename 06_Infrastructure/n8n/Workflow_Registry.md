...
4. **Not individually live-fired this pass, by design choice, not oversight:** the D32 duplicate-key branch, the D26 malformed-ID branch, and the D27 list/delete-failure branches are implemented and code-reviewed (gstack `/plan-eng-review`: 6 issues resolved interactively + 2 cross-model findings from the outside-voice pass, 8 total, all resolved) but weren't each deliberately triggered against the live production sheet/index, since doing so would mean deliberately corrupting a live third-party sheet or a live Pinecone call. Flagged here plainly, not glossed over.
